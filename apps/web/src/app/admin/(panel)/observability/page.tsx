  ...
  return (
    <div className="space-y-5">
      <section className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
        ...
        <StatTile label="Domain event" value={formatNumber(data.domainEvents.pendingCount + data.domainEvents.processingCount)} detail={`${formatNumber(data.domainEvents.deadLetterCount)} dead-letter · ${formatNumber(data.domainEvents.failedCount)} retry`} icon={<Radio className="h-4 w-4" />} tone="bg-amber-500/10 text-amber-300 ring-amber-500/20" />
        <StatTile label="Worker queue" value={formatNumber(workerTotal)} detail={`${formatNumber(data.workerJobs.retryScheduledCount)} retry · ${formatNumber(data.workerJobs.deadLetterCount)} dead-letter`} icon={<Server className="h-4 w-4" />} tone="bg-emerald-500/10 text-emerald-300 ring-emerald-500/20" />
        <StatTile label="Yetkilendirme" value={`${formatNumber(data.authorization.avgDurationMs)}ms`} detail={`${data.authorization.avgQueryCount} sorgu/istek · ${formatNumber(data.authorization.deniedCount)} red`} icon={<ShieldCheck className="h-4 w-4" />} tone="bg-violet-500/10 text-violet-300 ring-violet-500/20" />
      </section>

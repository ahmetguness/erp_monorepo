...
import {
  ...
  createLotSerial,
  assignLotToMovement,
  getLotSerialTraceability,
  type ListParams,
  type CreateLotSerialDTO,
  type TraceabilityParams,
} from '@/services/lot-serial.service';

const KEYS = {
  list: (p: ListParams) => ['lot-serials', p] as const,
  traceability: (p: TraceabilityParams) => ['lot-serials', 'traceability', p] as const,
};

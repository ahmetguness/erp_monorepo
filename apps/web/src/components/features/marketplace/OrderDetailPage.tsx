...
const STATUS_MAP: Record<string, { label: string; variant: 'neutral' | 'success' | 'warning' | 'danger' | 'info' }> = {
  PENDING: { label: 'Bekliyor', variant: 'warning' }, PROCESSING: { label: 'İşleniyor', variant: 'info' },
  SHIPPED: { label: 'Kargoda', variant: 'info' }, DELIVERED: { label: 'Teslim Edildi', variant: 'success' },
  CANCELLED: { label: 'İptal', variant: 'danger' }, RETURNED: { label: 'İade', variant: 'neutral' }, REFUNDED: { label: 'İade Edildi', variant: 'neutral' },
};
...
export function OrderDetailPage({ id }: { id: string }) {
  ...
  if (!order) return <div className="text-center py-20 text-slate-400">Sipariş bulunamadı.</div>;

  const s = STATUS_MAP[order.status];

  return (
    <div>
      <PageHeader title={`Sipariş ${order.externalId}`} subtitle={order.integration?.name ?? order.channel}
        action={<Button variant="ghost" size="sm" onClick={() => router.back()}><ArrowLeft className="w-4 h-4" />Geri</Button>} />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Durum', value: s ? <Badge variant={s.variant}>{s.label}</Badge> : order.status },
      ...
      {order.shippingAddress && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6">
          <h3 className="text-sm font-semibold text-white mb-2">Teslimat Adresi</h3>

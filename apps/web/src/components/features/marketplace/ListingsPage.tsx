...
export function ListingsPage() {
  ...
  const columns: ColumnDef<MarketplaceListing>[] = [
    ...
    },
    {
      ...
      width: "120px",
      render: (r) => (
        <Badge variant="info">{r.integration?.channel ?? "—"}</Badge>
      ),
    },
    {
      key: "externalId",
      ...
      width: "130px",
      render: (r) => (
        <code className="text-xs text-slate-400 bg-slate-800/60 px-2 py-0.5 rounded">
          {r.externalId}
        </code>
      ),
    },
    ...
    },
    {
      ...
      render: (r) => (
        <span
          className={`font-medium ${Number(r.stock) <= 0 ? "text-red-400" : "text-slate-300"}`}
        >
          {r.stock}
        </span>
      ),
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Pazaryerlerindeki ürün fiyat ve stok bilgilerini yönetin."
        action={
      ...
      >
        <div className="space-y-4">
          <Input
            label="Entegrasyon ID"
            required
            placeholder="Pazaryeri entegrasyon ID'si"

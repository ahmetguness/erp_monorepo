    ...
    items: [
      {
        ...
        description: "Tüm ürünleri sayfalı listeler",
      },
      {
        method: "GET",
        ...
        scope: "products:read",
        hasBody: false,
        description: "Tek ürün detayını getirir",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        hasBody: false,
        params: "?page=1&limit=5",
        description: "Müşteri ve tedarikçileri listeler",
      },
      {
        method: "GET",
    ...
    items: [
      {
        ...
        hasBody: false,
        params: "?page=1&limit=5",
        description: "Faturaları listeler",
      },
      {
        method: "GET",
        ...
        scope: "invoices:read",
        hasBody: false,
        description: "Fatura detayı ve satırları",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        description: "Depo bazlı stok seviyeleri",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        hasBody: false,
        params: "?page=1&limit=5",
        description: "Satış siparişlerini listeler",
      },
    ],
  },
];
...
export function ApiPlayground() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      setResponse(
        JSON.stringify(
          { error: "Bağlantı hatası. Backend çalışıyor mu?" },
          null,
          2,
        ),
      );
  ...
  }, [apiKey, endpoint, body, sandboxMode]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-0 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden min-h-[600px]">
      ...
      <div className="border-r border-slate-800 bg-slate-900/50 overflow-y-auto">
        ...
        <div className="p-4 border-b border-slate-800">
          <div className="flex items-center gap-2 mb-2">
            <div
              className={`w-2 h-2 rounded-full ${apiKey.trim() ? "bg-emerald-400 animate-pulse" : "bg-slate-600"}`}
            />
          ...
          <label className="mt-3 flex items-center justify-between gap-3 rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2">
            <span className="text-[11px] text-slate-400">Sandbox mode</span>
            <input
              type="checkbox"
              checked={sandboxMode}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-left transition-all mb-0.5 ${
                      isSelected
                        ? "bg-slate-800 ring-1 ring-sky-500/20"
                        : "hover:bg-slate-800/40"
                    }`}
                  >
                    <span
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${ms.bg} ${ms.text} w-11 text-center`}
                    >
                      {ep.method}
                    </span>
                    <div className="flex-1 min-w-0">
                      <span
                        className={`text-xs font-medium block ${isSelected ? "text-white" : "text-slate-400"}`}
                      >
                        {ep.label}
                      </span>
                    </div>
          ...
          <div className="border-t border-slate-800 p-3">
            ...
            <div className="space-y-1">
              {history.map((h, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 text-[10px] px-2 py-1 rounded-md bg-slate-800/30"
                >
                  <span
                    className={`font-bold ${h.status >= 200 && h.status < 300 ? "text-emerald-400" : "text-red-400"}`}
                  >
                    {h.status || "ERR"}
                  </span>
      ...
      <div className="flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-sky-500/5 border-b border-sky-500/10 text-[11px] text-sky-400/80">
          ...
          Sandbox modu acikken yazma istekleri dogrulanir, ancak veri degisikligi yapilmaz.
        </div>

        {/* Request header */}
        <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-800 bg-slate-900/80">
          <span
            className={`text-xs font-bold px-2.5 py-1 rounded-md ${methodStyle.bg} ${methodStyle.text}`}
          >
            {endpoint.method}
          </span>
          <code className="text-sm text-white font-mono flex-1 truncate">
            {endpoint.path}
            <span className="text-slate-600">{endpoint.params ?? ""}</span>
          </code>
          <Button
            size="sm"
            onClick={handleSend}
            ...
            disabled={
              !apiKey.trim() ||
              (endpoint.hasBody && body.trim() !== "" && !isValidJson(body))
            }
            className="gap-1.5"
          >
        ...
        <div className="px-5 py-3 border-b border-slate-800/50 flex items-center gap-4">
          ...
          <span className="ml-auto text-[10px] font-mono text-slate-600 bg-slate-800/60 px-2 py-0.5 rounded">
        ...
        <div className="flex-1 overflow-y-auto">
                ...
                <textarea
                  ...
                  className={`w-full bg-slate-950 border rounded-xl px-4 py-3 text-xs text-slate-300 font-mono resize-none focus:outline-none focus:ring-1 leading-relaxed ${
                    body.trim() && !isValidJson(body)
                      ...
                      : "border-slate-800 focus:border-sky-500/40 focus:ring-sky-500/20"
                  }`}
                />
            ...
            {statusCode !== null && (
              <div className="flex items-center gap-3 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-sm font-bold font-mono ${statusColor}`}
                  >
                    {statusCode || "ERR"}
                  </span>
                ...
                {response && (
                  <span className="text-[10px] text-slate-600">
                    {(new TextEncoder().encode(response).length / 1024).toFixed(
                      1,
                    )}{" "}
                    KB
                  </span>
            ...
            {response ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-xs text-slate-600">
                  Bir endpoint seçin ve API anahtarınızla istek gönderin
                </p>
              </div>
            )}
          </div>
          ...
          <div className="px-5 pb-5">
            ...
            <pre className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-[11px] text-slate-500 font-mono overflow-x-auto whitespace-pre leading-relaxed">
              {curlCommand}
            </pre>
          </div>
        </div>

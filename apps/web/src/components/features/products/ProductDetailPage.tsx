...
export function ProductDetailPage({ id }: Props) {
  ...
  if (!product) return <div className="text-slate-400 text-sm">Ürün bulunamadı.</div>;
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <Button variant="secondary" leftIcon={<Pencil className="w-4 h-4" />} onClick={() => router.push(`/dashboard/products/${id}/edit`)}>Düzenle</Button>
            <Button variant="danger" leftIcon={<Trash2 className="w-4 h-4" />} onClick={() => setDeleteOpen(true)}>Sil</Button>
          </div>
        }
      />

      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
        <div className="mb-4"><ActiveBadge isActive={product.isActive} /></div>
        ...
        <InfoRow label="KDV Oranı" value={product.taxRate ? `${product.taxRate.name} (%${product.taxRate.rate})` : null} />
        <InfoRow label="Barkod" value={product.barcode} />
        <InfoRow label="Alış Fiyatı" value={formatCurrency(product.purchasePrice)} />
        <InfoRow label="Satış Fiyatı" value={formatCurrency(product.salesPrice)} />
        <InfoRow label="Ort. Maliyet" value={formatCurrency(product.averageCost)} />
        <InfoRow label="Min. Stok" value={product.minStockLevel} />
        {product.description && <InfoRow label="Açıklama" value={product.description} />}
      </div>

      <div>
        <h2 className="text-sm font-semibold text-white mb-3">Stok Seviyeleri</h2>
        <DataTable

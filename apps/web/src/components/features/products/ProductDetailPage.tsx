...
function getMargin(product: Product): number | null {
  ...
  return (
    ((product.salesPrice - product.purchasePrice) / product.salesPrice) * 100
  );
}
...
function getStockState(
  ...
  if (minStock <= 0)
    return { label: "Minimum tanımsız", variant: "neutral", tone: "default" };
  if (totalStock <= 0)
    return { label: "Stokta yok", variant: "danger", tone: "danger" };
  if (totalStock < minStock)
    return { label: "Kritik stok", variant: "danger", tone: "danger" };
  if (totalStock <= minStock * 1.2)
    return { label: "Düşük stok", variant: "warning", tone: "warning" };
  ...
  return (
    <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 border-b border-slate-800/65 py-2.5 last:border-b-0">
      ...
      <div className="min-w-0 text-sm font-medium text-slate-200">
        {empty ? <span className="text-slate-600">Eksik</span> : value}
      </div>
    </div>
  );
}
    ...
    >
      <td className="px-4 py-3.5">
        <p className="font-medium text-slate-100">
          {row.warehouse?.name ?? "Depo bilgisi yok"}
        </p>
      ...
      <td className="px-4 py-3.5 text-right">
        <p className={cn("font-semibold tabular-nums", TONE_TEXT[tone])}>
          {formatQuantity(quantity, unitCode)}
        </p>
      ...
      <td className="px-4 py-3.5 text-right tabular-nums text-slate-500">
        {formatQuantity(min, unitCode)}
      </td>
      <td
        className={cn(
          "px-4 py-3.5 text-right font-semibold tabular-nums",
          diff < 0 ? "text-red-300" : "text-emerald-300",
        )}
      >
        {diff > 0 ? "+" : ""}
        {formatQuantity(diff, unitCode)}
      </td>
      ...
      <td className="px-4 py-3.5 text-center">
        <Badge
          variant={
            isOut || isCritical ? "danger" : isLow ? "warning" : "success"
          }
        >
          {isOut
            ? "Stokta yok"
      ...
      <td className="w-10 px-4 py-3.5 text-right">
        <ArrowRight className="h-4 w-4 text-slate-600 opacity-0 transition-all duration-150 group-hover:translate-x-0.5 group-hover:opacity-100" />
      </td>
    </tr>
  );
}
...
export function ProductDetailPage({ id }: Props) {
  ...
  const qualityFlags = useMemo(() => {
    ...
    if (!product.isActive)
      flags.push({ label: "Pasif ürün", variant: "neutral" });
    if (!product.category)
      flags.push({ label: "Kategori eksik", variant: "warning" });
    if (!product.barcode)
      flags.push({ label: "Barkod eksik", variant: "warning" });
    if (product.salesPrice <= 0 || product.purchasePrice <= 0)
      flags.push({ label: "Fiyat eksik", variant: "danger" });
    if (product.minStockLevel <= 0)
      flags.push({ label: "Min. stok eksik", variant: "warning" });
    if (stockSummary.gap > 0)
      flags.push({
        label: "Stok açığı var",
        variant: stockSummary.total <= 0 ? "danger" : "warning",
      });
    if (margin !== null && margin < 15)
      flags.push({
        label: "Düşük marj",
        variant: margin < 0 ? "danger" : "warning",
      });
    if (
      product.averageCost > product.purchasePrice &&
      product.purchasePrice > 0
    )
      flags.push({ label: "Maliyet yükselmiş", variant: "info" });
    return flags;
  }, [margin, product, stockSummary]);

  if (isLoading) return <FullPageSpinner />;
  if (!product || !stockSummary)
    return <div className="text-sm text-slate-400">Ürün bulunamadı.</div>;

  const unitCode = product.unit?.code;
  const purchaseDraft: RecommendedEntityAction[] =
    stockSummary.gap > 0
      ? [
          {
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <Link
          ...
          className="transition-colors duration-150 hover:text-sky-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/40"
        >
          Ürünler
        </Link>
        <span>/</span>
      ...
      <div className="flex flex-col gap-4 border-b border-slate-800/80 pb-5 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex min-w-0 gap-4">
          <EntityImage
            ...
            entityId={id}
            fallback="package"
            className="h-20 w-20 shrink-0 rounded-xl border border-slate-800"
          />
          <div className="min-w-0">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              <Badge variant={marginVariant}>
                {margin === null
                  ? "Marj yok"
                  : `%${formatNumber(margin, 1)} marj`}
              </Badge>
              <Badge variant={product.category ? "info" : "warning"}>
                {product.category?.name ?? "Kategori eksik"}
              </Badge>
            </div>
        ...
        <div className="flex flex-wrap gap-2 xl:justify-end">
          <Button
            ...
            onClick={() =>
              router.push(`/dashboard/stock/movements?productId=${id}`)
            }
          >
            Hareketler
          </Button>
          <Button
            variant="secondary"
            leftIcon={<Warehouse className="h-4 w-4" />}
            onClick={() =>
              router.push(`/dashboard/stock/levels?productId=${id}`)
            }
          >
            Stok
          </Button>
          <Button
            ...
            onClick={() =>
              router.push(`/dashboard/purchase-orders/requests?productId=${id}`)
            }
          >
            Talep
          </Button>
      ...
      <div className="grid rounded-xl border border-slate-800/80 bg-slate-950/35 sm:grid-cols-2 xl:grid-cols-4">
        {[
          {
            label: "Toplam Stok",
            value: formatQuantity(stockSummary.total, unitCode),
            hint: `Min. ${formatQuantity(stockSummary.min, unitCode)}`,
            tone: stockSummary.tone,
          },
          {
            label: "Eksik Miktar",
            value: formatQuantity(stockSummary.gap, unitCode),
            hint:
              stockSummary.gap > 0
                ? "Tamamlama gerekli"
                : "Tamamlama gerekmiyor",
            tone: stockSummary.gap > 0 ? "warning" : "success",
          },
          {
            label: "Birim Kar",
            ...
            hint:
              margin === null
                ? "Satış fiyatı yok"
                : `%${formatNumber(margin, 1)} brüt marj`,
            tone:
              profitPerUnit < 0
                ? "danger"
                : profitPerUnit === 0
          ...
          },
          {
            ...
            hint: `Alış ${formatCurrency(product.purchasePrice)}`,
            tone: "info",
          },
        ].map((metric) => (
          <div
            key={metric.label}
            className="border-b border-slate-800/70 px-4 py-3 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0"
          >
            <p
              className={cn(
                "text-base font-semibold tabular-nums",
                TONE_TEXT[metric.tone as Tone],
              )}
            >
              {metric.value}
            </p>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-white">
                  Kontrol gerektiren alanlar
                </p>
                <p className="mt-0.5 text-xs text-slate-500">
                  Ürün kartındaki eksikler stok, satış ve satın alma kararlarını
                  etkileyebilir.
                </p>
              </div>
            </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <main className="space-y-5">
          <Section
            ...
            description="Depo bazında güncel miktar ve minimum stok karşılaştırması."
            ...
            <div className="mb-4">
              <div className="h-2 overflow-hidden rounded-full bg-slate-900">
                <div
                  className={cn(
                    "h-full rounded-full",
                    stockSummary.gap > 0 ? "bg-amber-400" : "bg-emerald-400",
                  )}
                  style={{ width: `${stockSummary.coverage}%` }}
                />
              </div>
                    ...
                    <tr className="border-b border-slate-800/80">
                      {[
                        ...
                        <th
                          key={header || index}
                          className={cn(
                            "px-4 py-3 text-left text-[11px] font-semibold uppercase text-slate-500",
                            [1, 2, 3].includes(index) && "text-right",
                            index === 5 && "text-center",
                          )}
                        >
                          {header}
                  ...
                  <tbody>
                    {loadingStock ? (
                      Array.from({ length: 4 }).map((_, row) => (
                        <tr
                          key={row}
                          className="border-b border-slate-800/45 last:border-0"
                        >
                          {Array.from({ length: 7 }).map((__, col) => (
                            <td key={col} className="px-4 py-3.5">
                              <div className="h-3.5 w-2/3 animate-pulse rounded bg-slate-800/75" />
                            </td>
                          ))}
                        </tr>
                      ))
                      ...
                      <tr>
                        <td colSpan={7} className="px-4 py-10 text-center">
                          ...
                          <p className="mt-1 text-sm text-slate-500">
                            Bu ürün için depo bazlı stok seviyesi henüz
                            oluşmamış.
                          </p>
                        </td>
                      </tr>
                      ...
                      stockLevels.map((row) => (
                        <StockRow
                          key={row.id}
                          row={row}
                          min={stockSummary.min}
                          unitCode={unitCode}
                          onOpen={() =>
                            router.push(
                              `/dashboard/stock/levels?productId=${id}`,
                            )
                          }
                        />
                      ))
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <Section
              title="Katalog Bilgisi"
              icon={<Tags className="h-4 w-4" />}
            >
              <InfoRow
                label="Ürün kodu"
                value={
                  <span className="font-mono text-sky-300">{product.code}</span>
                }
              />
              ...
              <InfoRow
                ...
                value={
                  product.taxRate
                    ? `%${formatNumber(product.taxRate.rate, 0)}`
                    : null
                }
              />
              ...
              <InfoRow
                label="Durum"
                value={<ActiveBadge isActive={product.isActive} />}
              />
            </Section>

            <Section
              title="Fiyat ve Maliyet"
            ...
            >
              ...
              <InfoRow
                ...
                value={
                  <span
                    className={cn(
                      profitPerUnit < 0 ? "text-red-300" : "text-emerald-300",
                    )}
                  >
          ...
          >
            <div className="grid gap-x-6 md:grid-cols-2">
              <InfoRow
                label="Emniyet stoğu"
                value={
                  product.safetyStock === null
                    ? null
                    : formatNumber(product.safetyStock, 3)
                }
              />
              <InfoRow
                ...
                value={
                  product.reorderPoint === null
                    ? null
                    : formatNumber(product.reorderPoint, 3)
                }
              />
              <InfoRow
                ...
                value={
                  product.reorderQty === null
                    ? null
                    : formatNumber(product.reorderQty, 3)
                }
              />
              <InfoRow
                label="Temin süresi"
          ...
          <Section
            title="Ürün Görseli"
            description="Katalog, satış ve stok ekranlarında kullanılacak ürün görselini yönetin."
        ...
        <aside className="space-y-5">
          ...
          >
            <InfoRow
              label="Stok durumu"
              value={
                <Badge variant={stockSummary.variant}>
                  {stockSummary.label}
                </Badge>
              }
            />
            ...
            <InfoRow
              ...
              value={
                qualityFlags.length === 0 ? (
                  <Badge variant="success">Kart tamam</Badge>
                ) : (
                  `${qualityFlags.length} kontrol`
                )
              }
            />
          </Section>
          ...
          <Section title="Hızlı Geçişler" icon={<Boxes className="h-4 w-4" />}>
            <div className="grid gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/purchase-orders/requests?productId=${id}`,
                  )
                }
              >
          ...
          >
            ...
            />
            <InfoRow
              label="Satış - ort. maliyet"
              value={formatCurrency(product.salesPrice - product.averageCost)}
            />
          </Section>
      ...
      <ConfirmDialog
        ...
        message={
          <div className="space-y-2">
            ...
            <p>
              Bu işlem stok, teklif, sipariş veya fatura geçmişinde kullanılan
              kayıtlarla ilişkili olabilir. Aktif kullanımdaki ürünlerde silmek
              yerine pasifleştirme daha güvenli olabilir.
            </p>
          </div>
        }

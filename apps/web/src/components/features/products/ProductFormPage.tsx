...
import { z } from 'zod';
import {
  ArrowLeft, Package, Tag, DollarSign, Warehouse,
  Save, X, Barcode, Hash, Check, Eye,
  TrendingUp, TrendingDown, Layers, AlertCircle,
} from 'lucide-react';
...
function SectionCard({ step, icon, iconColor, title, description, done, children }: {
  step: number; icon: React.ReactNode; iconColor: string; title: string;
  description?: string; done?: boolean; children: React.ReactNode;
}) {
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-800/60">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-600">Adım {step}</span>
            {done && <span className="text-[10px] font-medium text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded-full">Tamam</span>}
          </div>
...
function LivePreview({ name, code, category, purchasePrice, salesPrice, unit }: {
  name: string; code: string; category: string; purchasePrice: string; salesPrice: string; unit: string;
}) {
  const hasData = name || code;
  const margin = Number(salesPrice || 0) - Number(purchasePrice || 0);
  const marginPct = Number(purchasePrice || 0) > 0 ? ((margin / Number(purchasePrice)) * 100).toFixed(1) : '0.0';

  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-800/60">
        <Eye className="w-3.5 h-3.5 text-slate-500" />
        <span className="text-xs font-semibold text-slate-400">Canlı Önizleme</span>
      </div>
      <div className="p-4">
        {!hasData ? (
          <div className="py-8 text-center">
            <Package className="w-8 h-8 text-slate-700 mx-auto mb-2" />
            <p className="text-xs text-slate-600">Bilgileri girdikçe burada görünecek</p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white truncate">{name || 'Ürün Adı'}</p>
                  <p className="text-xs text-slate-500 font-mono">{code || 'KOD'}</p>
                </div>
              </div>
              {category && (
                <div className="mt-3">
            ...
            {/* Price summary */}
            {(Number(purchasePrice || 0) > 0 || Number(salesPrice || 0) > 0) && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5 text-slate-500">
                    <TrendingDown className="w-3 h-3 text-red-400" />Alış
                  </span>
                  <span className="font-semibold text-slate-300">{formatCurrency(Number(purchasePrice || 0))}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5 text-slate-500">
                    <TrendingUp className="w-3 h-3 text-emerald-400" />Satış
                  </span>
                  <span className="font-semibold text-slate-300">{formatCurrency(Number(salesPrice || 0))}</span>
                </div>
                <div className="h-px bg-slate-800 my-1" />
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500">Kar Marjı</span>
                  <span className={cn('font-bold', margin >= 0 ? 'text-emerald-400' : 'text-red-400')}>
            ...
            {unit && (
              <div className="text-[10px] text-slate-600 text-center pt-1">Birim: {unit}</div>
            )}
          </div>
        )}
      </div>
...
export function ProductFormPage({ editId }: Props) {
  ...
  const step2Done = !!(Number(watchAll.purchasePrice || 0) > 0 || Number(watchAll.salesPrice || 0) > 0);
  const step3Done = !!(Number(watchAll.initialStock || 0) > 0 && watchAll.warehouseId);
  const completedSteps = [step1Done, step2Done, ...(isEdit ? [] : [step3Done])].filter(Boolean).length;
  const totalSteps = isEdit ? 2 : 3;

  // Resolve display names for preview
  const selectedUnit = useMemo(() => units.find((u) => u.id === watchAll.unitId), [units, watchAll.unitId]);
  const selectedCategory = useMemo(() => categories.find((c) => c.id === watchAll.categoryId), [categories, watchAll.categoryId]);

  useEffect(() => {
    if (existing) {
      reset({
        code: existing.code, name: existing.name, unitId: existing.unitId,
        categoryId: existing.categoryId ?? '', taxRateId: existing.taxRateId ?? '',
        barcode: existing.barcode ?? '', description: existing.description ?? '',
        purchasePrice: String(existing.purchasePrice), salesPrice: String(existing.salesPrice),
  ...
  const onSubmit = async (data: ProductForm) => {
    const payload = {
      code: data.code, name: data.name, unitId: data.unitId,
      categoryId: data.categoryId || undefined, taxRateId: data.taxRateId || undefined,
          ...
          if (qty > 0 && wId) {
            try {
              await createManualMovement({
                productId: p.id, type: 'OPENING', quantity: qty,
                warehouseId: wId, unitCost: payload.purchasePrice,
    ...
    <div>
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 ml-[38px]">
                {isEdit ? 'Mevcut ürün bilgilerini güncelleyin.' : 'Kataloğunuza yeni bir ürün ekleyin — adımları takip edin.'}
              </p>
            </div>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-3">
            <div className="text-right">
              ...
              <p className="text-sm font-bold text-white">{completedSteps}/{totalSteps}</p>
            </div>
      ...
      <div className="flex gap-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex-1 min-w-0 space-y-5">
          ...
          <SectionCard step={1} icon={<Tag className="w-4 h-4 text-sky-400" />} iconColor="bg-sky-500/10"
            title="Temel Bilgiler" description="Ürün kodu, adı ve sınıflandırma" done={step1Done}>
            <FormRow cols={2}>
              <Input label="Ürün Kodu" required placeholder="P001" error={errors.code?.message}
                prefixIcon={<Hash className="w-3.5 h-3.5" />} {...register('code')} />
              <Input label="Ürün Adı" required placeholder="Ürün adını girin" error={errors.name?.message}
                prefixIcon={<Package className="w-3.5 h-3.5" />} {...register('name')} />
            </FormRow>
            <FormRow cols={2}>
              <Select label="Birim" required options={unitOptions} placeholder="Birim seçin"
                error={errors.unitId?.message} {...register('unitId')} />
              <Combobox label="Kategori" options={categoryOptions}
                value={watch('categoryId') ?? ''} onChange={(v) => setValue('categoryId', v)}
                onCreateNew={(name) => { createCategory.mutate({ name }, { onSuccess: (c) => setValue('categoryId', c.id) }); }}
                createLabel="Yeni kategori" placeholder="Kategori seçin veya yazın…" />
            </FormRow>
            <FormRow cols={2}>
              <Input label="Barkod" placeholder="1234567890"
                prefixIcon={<Barcode className="w-3.5 h-3.5" />} {...register('barcode')} />
              <Select label="KDV Oranı" options={taxRateOptions} {...register('taxRateId')} />
            </FormRow>
            <Textarea label="Açıklama" placeholder="Ürün hakkında kısa bir açıklama…" {...register('description')} />
          </SectionCard>

          {/* ── Step 2: Fiyatlandırma ───────────── */}
          <SectionCard step={2} icon={<DollarSign className="w-4 h-4 text-emerald-400" />} iconColor="bg-emerald-500/10"
            title="Fiyatlandırma" description="Alış, satış fiyatı ve minimum stok seviyesi" done={step2Done}>
            <FormRow cols={3}>
              ...
              <Input label="Min. Stok Seviyesi" type="number" step="0.001" placeholder="0" {...register('minStockLevel')} />
            </FormRow>
          </SectionCard>

          {/* ── Step 3: Başlangıç Stoğu ─────────── */}
          {!isEdit && (
            <SectionCard step={3} icon={<Warehouse className="w-4 h-4 text-violet-400" />} iconColor="bg-violet-500/10"
              title="Başlangıç Stoğu" description="Opsiyonel — ürünü bir depoya başlangıç stoğu ile ekleyin" done={step3Done}>
              <FormRow cols={2}>
                <Select label="Depo" options={warehouseOptions} {...register('warehouseId')} />
                <Input label="Miktar" type="number" step="0.001" placeholder="0"
                  helperText="Boş bırakırsanız stok girişi yapılmaz." {...register('initialStock')} />
              </FormRow>
            </SectionCard>
          )}
          ...
          <div className="sticky bottom-0 z-20 -mx-1 px-1 pb-4 pt-3 bg-gradient-to-t from-slate-950 via-slate-950/95 to-transparent">
            <div className="flex items-center justify-between bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-xl px-5 py-3">
              <div className="flex items-center gap-2 text-xs text-slate-500">
                {errorCount > 0 ? (
                  <span className="flex items-center gap-1 text-red-400">
                    <AlertCircle className="w-3.5 h-3.5" />{errorCount} hata düzeltilmeli
                  </span>
                ) : step1Done ? (
                  <span className="flex items-center gap-1 text-emerald-400">
                    <Check className="w-3.5 h-3.5" />Kaydetmeye hazır
                  </span>
                ) : (
              ...
              <div className="flex items-center gap-2.5">
                ...
                <Button type="submit" size="sm" loading={isPending} leftIcon={<Save className="w-3.5 h-3.5" />}
        ...
        <div className="hidden lg:block w-72 shrink-0">
          <div className="sticky top-4 space-y-4">
            <LivePreview
              ...
              unit={selectedUnit ? `${selectedUnit.name} (${selectedUnit.code})` : ''}
            />

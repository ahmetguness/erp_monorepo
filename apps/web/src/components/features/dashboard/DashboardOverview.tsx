    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 hover:border-slate-700 transition-colors">
      ...
      {(trend || sub) && (
        <div className="flex items-center gap-2 mt-2 pl-12">
          {trend && (
            <span
              className={`flex items-center gap-0.5 text-[11px] font-semibold px-1.5 py-0.5 rounded-full ${trend.positive ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
            >
              {trend.positive ? (
...
function QuickBtn({
  ...
  return (
    <Link
      href={href}
      className={`relative flex items-center gap-2.5 px-4 py-2.5 rounded-xl border ${ring} bg-slate-800/40 backdrop-blur-sm hover:bg-slate-800/70 transition-all duration-200 group`}
    >
      <div
        className={`p-1.5 rounded-lg ${color} group-hover:scale-110 transition-transform duration-200`}
      >
        {icon}
      </div>
...
export function DashboardOverview() {
  ...
  const { data: invs } = useQuery({
    queryKey: ["dash", "invs"],
    queryFn: async () => {
      const r = await apiClient.get("/api/invoices", {
  ...
  const areaData = [
    ...
    {
      month: "Mar",
      gelir: rev?.totalGross ?? 40000,
      gider: exp?.totalGross ?? 25000,
    },
    { month: "Nis", gelir: 0, gider: 0 },
  ].filter((d) => d.gelir > 0 || d.gider > 0);

  // Top debtors
  const debtors = (bal?.contacts ?? [])
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="relative z-10 p-6 flex flex-col lg:flex-row lg:items-center gap-5">
          ...
          <div className="flex flex-wrap gap-2 lg:justify-end">
            ...
            <QuickBtn
              ...
              icon={<BarChart3 className="w-3.5 h-3.5 text-pink-400" />}
              label="Rapor"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          label="Bu Ay Gelir"
          value={formatCurrency(rev?.totalGross ?? 0)}
          sub={`${rev?.invoiceCount ?? 0} fatura`}
          icon={<TrendingUp className="w-4 h-4 text-emerald-400" />}
          accent="bg-emerald-500/10"
        />
        <StatCard
          label="Bu Ay Gider"
          value={formatCurrency(exp?.totalGross ?? 0)}
          sub={`${exp?.invoiceCount ?? 0} fatura`}
        ...
        <StatCard
          label="Stok Değeri"
          value={formatCurrency(stk?.summary.totalStockValue ?? 0)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-5">
          ...
          {pieData.length > 0 ? (
            ...
            <div className="h-36 flex items-center justify-center text-sm text-slate-600">
              Veri yok
            </div>
          )}
        </div>
                ...
                >
                  <div
                    className={cn(
                      ...
                      STATUS_DOT[inv.status] ?? "bg-slate-500",
                    )}
                  />
                  <div className="flex-1 min-w-0">
            ...
            {debtors.length > 0 && (
              <div className="space-y-1.5">
                {debtors.map((c) => (
                  ...
                  >
                    <div className="w-5 h-5 rounded-full bg-sky-500/10 flex items-center justify-center text-[10px] font-bold text-sky-400 shrink-0">
                      {c.name.charAt(0)}
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-end gap-3">
                      <div className="text-right">
                        ...
                        <p className="text-xs font-bold text-white tabular-nums">
                          ₺{item.cur.forexBuying.toFixed(4)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-[9px] text-red-500">Satış</p>
                        <p className="text-xs font-bold text-white tabular-nums">
                          ₺{item.cur.forexSelling.toFixed(4)}
                        </p>
                      </div>
                    </div>
                  ) : (

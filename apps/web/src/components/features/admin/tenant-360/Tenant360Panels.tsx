...
function DataTable({ headers, rows }: { headers: string[]; rows: ReactNode[][] }) {
  return rows.length ? <div className="overflow-x-auto rounded-lg border border-slate-800"><table className="w-full text-left text-sm text-slate-300"><thead className="bg-slate-800/60 text-xs text-slate-400"><tr>{headers.map(header => <th className="border-b border-slate-800 p-3" key={header} scope="col">{header}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={index} className="transition-colors hover:bg-slate-800/40">{row.map((cell, column) => <td className="border-b border-slate-800 p-3 align-top" key={column}>{cell}</td>)}</tr>)}</tbody></table></div> : <p className="py-4 text-slate-400">Kayıt bulunamadı.</p>;
}
...
export function Tenant360Panels({ tab, data }: { tab: Tenant360Tab; data: Tenant360Snapshot }) {
  switch (tab) {
    ...
    case 'Operasyon': return data.operations === null ? denied : <div className="space-y-5"><h3>Kuyruk kayıtları (tüm saklanan kayıtlar)</h3><DataTable headers={['Kaynak', 'Durum', 'Adet']} rows={data.operations.queue.map(row => [row.source, row.status, row.count])} /><h3>Son 20 başarısız iş / olay</h3><p className="text-sm text-slate-400">Hassas veri içerebilen ham hata mesajları ve iş yükleri gösterilmez.</p><DataTable headers={['Kaynak', 'İş / olay', 'Durum', 'Deneme', 'Güncelleme']} rows={data.operations.recentFailures.map(row => [row.source, <span key={row.id} title={row.id}>{row.name}</span>, row.status, row.attempts, date(row.updatedAt)])} /></div>;

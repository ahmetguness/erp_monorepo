...
const contactSchema = z.object({
  ...
  taxNumber: z.string()
    .optional()
    .refine((v) => !v || /^\d{10,11}$/.test(v), { message: 'Vergi no 10 veya 11 haneli olmalıdır' }),
  taxOffice: z.string().optional(),
  email: z.string().email('Geçerli bir e-posta adresi girin').optional().or(z.literal('')),
  phone: z.string()
    .optional()
    .refine((v) => !v || /^[\d\s\+\-\(\)]{7,20}$/.test(v), { message: 'Geçerli bir telefon numarası girin' }),
...
export function ContactFormPage({ editId }: Props) {
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isDirty } } = useForm<ContactFormData>({
  ...
  useEffect(() => {
    if (existing) {
      const termStr = existing.paymentTermDays != null ? String(existing.paymentTermDays) : '';
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-5xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
          ...
          <div className="lg:col-span-3 space-y-5">
            {/* Basic Info */}
            <FormSection title="Temel Bilgiler" description="Cari hesap tipi ve kimlik bilgileri">
              <FormRow cols={3}>
                <div>
                  <Select label="Tip" required options={TYPE_OPTIONS} error={errors.type?.message} {...register('type')} />
                  <p className="text-[10px] text-slate-500 mt-1">{TYPE_HINTS[watchType] ?? ''}</p>
                </div>
                <Input label="Ad / Ünvan" required placeholder="Şirket veya kişi adı" error={errors.name?.message} {...register('name')} />
                <Input
                  label="Hesap Kodu"
                  placeholder={isEdit ? '' : 'Otomatik oluşturulur'}
                  helperText={!isEdit ? 'Boş bırakırsanız otomatik atanır (ör: MUS00001)' : undefined}
                  error={errors.code?.message}
                  {...register('code')}
                />
              </FormRow>
              <FormRow cols={2}>
                <Input label="Vergi / TC Kimlik No" placeholder="1234567890" error={errors.taxNumber?.message} {...register('taxNumber')} />
            ...
            <FormSection title="İletişim Bilgileri">
              <FormRow cols={3}>
                ...
                <Input label="Telefon" placeholder="+90 (555) 000 00 00" error={errors.phone?.message} {...register('phone')} />
                <Input label="Website" placeholder="https://sirket.com" error={errors.website?.message} {...register('website')} />
              </FormRow>
              <Input label="Adres" placeholder="Mahalle, Sokak, No, Bina" error={errors.address?.message} {...register('address')} />
              <FormRow cols={2}>
            ...
            {/* Commercial Terms */}
            <FormSection title="Ticari Koşullar" description="Kredi limiti ve ödeme vade bilgileri">
              <FormRow cols={2}>
                <Input label="Kredi Limiti (₺)" type="number" min="0" step="0.01" placeholder="50000" error={errors.creditLimit?.message} {...register('creditLimit')} />
                <div>
                  {usePresetTerm ? (
                    <Select
                      label="Ödeme Vadesi"
                      ...
                      onChange={(e) => {
                        ...
                        if (v === '') {
                          ...
                          setValue('paymentTermDays', '', { shouldDirty: true });
                        } else {
                    ...
                    <div>
                      <Input label="Ödeme Vadesi (gün)" type="number" min="0" placeholder="45" error={errors.paymentTermDays?.message} {...register('paymentTermDays')} />
                      <button type="button" onClick={() => setUsePresetTerm(true)} className="text-[10px] text-sky-400 hover:text-sky-300 mt-1">
            ...
            <FormSection title="Notlar">
              <Textarea placeholder="Ek notlar, özel anlaşmalar, dikkat edilecek hususlar…" rows={3} {...register('notes')} />
          ...
          <div className="space-y-5">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Etiketler</h3>
              {tags.length > 0 && (
              ...
              <div className="flex gap-1.5">
                ...
                />
                <Button type="button" variant="ghost" size="sm" onClick={() => addTag(tagInput)} className="shrink-0">
            ...
            {isEdit && existing && (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Hesap Bilgisi</h3>
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-500">Durum</span>
                    <span className={existing.isActive ? 'text-emerald-400' : 'text-slate-500'}>{existing.isActive ? 'Aktif' : 'Pasif'}</span>
                  </div>
                  {existing.financials && (
                    <>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Bakiye</span>
                        <span className={existing.financials.currentBalance >= 0 ? 'text-emerald-400 font-medium' : 'text-red-400 font-medium'}>
                          {existing.financials.currentBalance >= 0 ? '+' : ''}{existing.financials.currentBalance.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Açık Fatura</span>
                        <span className="text-slate-300">{existing.financials.openInvoiceCount}</span>
                      </div>
                    </>
                  )}
                </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2 lg:sticky lg:top-6">
              <Button type="submit" loading={isPending} className="w-full" leftIcon={<Save className="w-4 h-4" />}>
              ...
              {isDirty && (
                <p className="text-[10px] text-amber-400 text-center">Kaydedilmemiş değişiklikler var</p>
              )}
            </div>
          </div>
        </div>

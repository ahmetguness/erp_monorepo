    ...
    <div>
      ...
      <div className="max-w-2xl">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center space-y-4">
          <div className={`w-12 h-12 rounded-xl mx-auto flex items-center justify-center ${isReceive ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
            <Icon className={`w-6 h-6 ${isReceive ? 'text-emerald-400' : 'text-red-400'}`} />
          </div>
          <h2 className="text-lg font-semibold text-white">{title}</h2>
          <p className="text-sm text-slate-400">
            Bu sayfa yakında aktif olacak. Şu an ödeme işlemleri Ödemeler modülünden yapılabilir.
          </p>
          <div className="flex gap-3 justify-center pt-2">
            <Button variant="secondary" onClick={() => router.push('/dashboard/payments')}>

...
import { z } from 'zod';
import {
  Plus, Trash2, Save, X, FileText, CalendarDays,
  StickyNote, BookOpen, ArrowUpRight, ArrowDownRight,
  Scale, Check, AlertCircle, CheckCircle, Pencil, RotateCcw,
} from 'lucide-react';
...
export function JournalEntriesPage() {
  ...
  const updateEntry = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { date: string; description?: string; lines: { accountId: string; debit: number; credit: number; description?: string }[] } }) =>
  ...
  const columns: ColumnDef<JournalEntry>[] = [
    {
      key: 'number', header: 'Fiş No', width: '120px',
      render: (r) => (
        <div>
          <span className="font-mono text-sky-400">{r.number}</span>
          {r.description?.startsWith('Ters kayıt') && (
            <span className="ml-1.5 text-[9px] font-medium text-amber-400 bg-amber-500/10 px-1 py-0.5 rounded">STORNO</span>
          )}
        </div>
      ),
    },
    { key: 'date', header: 'Tarih', width: '100px', render: (r) => <span className="text-slate-400 text-xs">{formatDate(r.date)}</span> },
    {
      ...
      render: (r) => <span className="text-slate-300 text-sm truncate block max-w-[200px]">{r.description ?? '—'}</span>,
    },
    {
      key: 'totalDebit', header: 'Borç', width: '110px', align: 'right',
      render: (r) => {
        const total = r.lines?.reduce((s, l) => s + Number(l.debit), 0) ?? 0;
        return <span className="text-emerald-400 font-medium text-sm tabular-nums">{total > 0 ? formatCurrency(total) : '—'}</span>;
      },
    },
    {
      key: 'totalCredit', header: 'Alacak', width: '110px', align: 'right',
      render: (r) => {
        const total = r.lines?.reduce((s, l) => s + Number(l.credit), 0) ?? 0;
        return <span className="text-red-400 font-medium text-sm tabular-nums">{total > 0 ? formatCurrency(total) : '—'}</span>;
      },
    },
    {
      key: 'lineCount', header: 'Satır', width: '60px', align: 'center',
      render: (r) => <span className="text-xs text-slate-500">{r.lines?.length ?? 0}</span>,
    },
    {
      key: 'isPosted', header: 'Durum', width: '90px', align: 'center',
        ...
        <div className="flex items-center justify-end gap-1.5">
          {!r.isPosted ? (
            <>
              ...
              <button type="button"
                onClick={(e) => { e.stopPropagation(); postEntry.mutate(r.id); }}
                className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium
            ...
            <button type="button"
              onClick={(e) => { e.stopPropagation(); reverseEntry.mutate(r.id); }}
              className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        {([
          { key: 'all' as const, label: 'Tümü', color: 'text-sky-400', active: 'bg-sky-500/15 text-sky-400 border-sky-500/30' },
          { key: 'draft' as const, label: 'Taslak', color: 'text-amber-400', active: 'bg-amber-500/15 text-amber-400 border-amber-500/30' },
          { key: 'posted' as const, label: 'Onaylı', color: 'text-emerald-400', active: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' },
        ]).map((f) => {
          const allData = data?.data ?? [];
          const count = f.key === 'all' ? allData.length : f.key === 'draft' ? allData.filter((e) => !e.isPosted).length : allData.filter((e) => e.isPosted).length;
          return (
      ...
      <Modal isOpen={createOpen} onClose={closeModal} title="Yeni Yevmiye Fişi"
        description="Borç ve alacak toplamları eşit olmalıdır." size="lg"
        footer={
          <>
            <Button variant="ghost" size="sm" leftIcon={<X className="w-3.5 h-3.5" />} onClick={closeModal}>İptal</Button>
            <Button size="sm" loading={createEntry.isPending} disabled={!isBalanced}
              leftIcon={isBalanced ? <Save className="w-3.5 h-3.5" /> : <AlertCircle className="w-3.5 h-3.5" />}
              ...
              className={isBalanced
                ...
                : ''}>
              {isBalanced ? 'Fişi Kaydet' : `Dengesiz (${formatCurrency(diff)})`}
            </Button>
          </>
        }
      >
        <form className="space-y-5">
          ...
          <div className="bg-slate-800/30 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 rounded-lg bg-sky-500/10"><FileText className="w-3.5 h-3.5 text-sky-400" /></div>
              <span className="text-xs font-semibold text-white">Fiş Bilgileri</span>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <Input label="Açıklama" placeholder="Fiş açıklaması…"
                prefixIcon={<StickyNote className="w-3.5 h-3.5" />} {...register('description')} />
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="p-1.5 rounded-lg bg-violet-500/10"><BookOpen className="w-3.5 h-3.5 text-violet-400" /></div>
                <span className="text-xs font-semibold text-white">Fiş Satırları</span>
              </div>
              ...
              {fields.map((field, idx) => {
                ...
                return (
                  <div key={field.id} className={cn(
                    ...
                    isDebit ? 'border-emerald-500/10' : isCredit ? 'border-red-500/10' : 'border-slate-800',
                  )}>
                    {/* Row number */}
                    ...
                    <div className="grid grid-cols-12 gap-2.5 items-start ml-3">
                      ...
                      <div className="col-span-4">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Hesap</label>
                        <select
                          className={cn(
                            ...
                            errors.lines?.[idx]?.accountId ? 'border-red-500' : 'border-slate-700',
                          )}
                      ...
                      <div className="col-span-2">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1 flex items-center gap-1">
                          <ArrowUpRight className="w-2.5 h-2.5 text-emerald-500" />Borç
                        </label>
                      ...
                      <div className="col-span-2">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1 flex items-center gap-1">
                          <ArrowDownRight className="w-2.5 h-2.5 text-red-500" />Alacak
                        </label>
                        <input type="number" step="1" min="0" placeholder="0"
                          className={cn(
                            'w-full bg-slate-800 border border-slate-700 rounded-lg text-sm text-right tabular-nums px-2.5 py-2',
                      ...
                      <div className="col-span-3">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Açıklama</label>
                        <input placeholder="Satır notu…"
          ...
          <div className={cn(
            ...
            isBalanced ? 'bg-emerald-500/5 border-emerald-500/15' : 'bg-red-500/5 border-red-500/15',
          )}>
            <div className={cn('p-2 rounded-lg', isBalanced ? 'bg-emerald-500/10' : 'bg-red-500/10')}>
              {isBalanced
            ...
            <div className="flex-1 grid grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-sm font-bold text-emerald-400 tabular-nums">{formatCurrency(totalDebit)}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Toplam Alacak</p>
                <p className="text-sm font-bold text-red-400 tabular-nums">{formatCurrency(totalCredit)}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Fark</p>
                <p className={cn('text-sm font-bold tabular-nums', isBalanced ? 'text-emerald-400' : 'text-red-400')}>
...
function EditJournalEntryModal({ entry, accounts, onClose, onSave, isPending }: {
  entry: JournalEntry;
  accounts: { id: string; code: string; name: string }[];
  onClose: () => void;
  onSave: (data: { date: string; description?: string; lines: { accountId: string; debit: number; credit: number; description?: string }[] }) => void;
  isPending: boolean;
}) {
  const accountOptions = [
    { value: '', label: '— Hesap seçin —' },
  ...
  const { register, control, handleSubmit, watch, formState: { errors } } = useForm<EditForm>({
  ...
  return (
    <Modal isOpen onClose={onClose} title={`Fiş Düzenle — ${entry.number}`}
      description="Taslak fişi düzenleyin." size="lg"
      footer={
        <>
          <Button variant="ghost" size="sm" leftIcon={<X className="w-3.5 h-3.5" />} onClick={onClose}>İptal</Button>
          <Button size="sm" loading={isPending} disabled={!isBalanced}
            leftIcon={isBalanced ? <Save className="w-3.5 h-3.5" /> : <AlertCircle className="w-3.5 h-3.5" />}
      ...
      <form className="space-y-5">
        ...
        <div className="bg-slate-800/30 border border-slate-800 rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2 mb-1">
            <div className="p-1.5 rounded-lg bg-sky-500/10"><FileText className="w-3.5 h-3.5 text-sky-400" /></div>
            <span className="text-xs font-semibold text-white">Fiş Bilgileri</span>
            <Badge variant="warning" className="ml-auto">Taslak</Badge>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Input label="Tarih" required type="date" prefixIcon={<CalendarDays className="w-3.5 h-3.5" />} {...register('date')} />
            <Input label="Açıklama" placeholder="Fiş açıklaması…" prefixIcon={<StickyNote className="w-3.5 h-3.5" />} {...register('description')} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {fields.map((field, idx) => {
              ...
              return (
                <div key={field.id} className={cn(
                  ...
                  isDebit ? 'border-emerald-500/15' : isCredit ? 'border-red-500/15' : 'border-slate-800',
                )}>
                  ...
                  <div className="ml-4 space-y-3">
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5 block">Hesap</label>
                        <select
                          className={cn(
                      ...
                      <div>
                        ...
                        <input type="number" step="1" min="0" placeholder="0"
                          className={cn(
                            ...
                            'focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-colors',
                            isDebit ? 'text-emerald-400 border-emerald-500/20' : 'text-white',
                          )}
                          {...register(`lines.${idx}.debit`)} />
                      </div>
                      <div>
                        ...
                        <input type="number" step="1" min="0" placeholder="0"
                          className={cn(
                            'w-full bg-slate-800 border border-slate-700 rounded-lg text-sm text-right tabular-nums px-3 py-2',
                            'focus:outline-none focus:ring-2 focus:ring-red-500 transition-colors',
                            isCredit ? 'text-red-400 border-red-500/20' : 'text-white',
                          )}
                          {...register(`lines.${idx}.credit`)} />
                      </div>
                      <div>
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                          <StickyNote className="w-2.5 h-2.5" />Açıklama
                        </label>
                        <input placeholder="Satır notu…"
            ...
            <button type="button" onClick={() => append({ accountId: '', debit: '0', credit: '0' })}
        ...
        <div className={cn(
          ...
          <div className={cn('p-2.5 rounded-lg', isBalanced ? 'bg-emerald-500/10' : 'bg-red-500/10')}>
            {isBalanced ? <Check className="w-5 h-5 text-emerald-400" /> : <Scale className="w-5 h-5 text-red-400" />}
          </div>
          <div className="flex-1 grid grid-cols-3 gap-4">
            <div>
              <p className="text-[10px] text-slate-500 uppercase tracking-wider">Toplam Borç</p>
              <p className="text-base font-bold text-emerald-400 tabular-nums">{formatCurrency(totalDebit)}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase tracking-wider">Toplam Alacak</p>
              <p className="text-base font-bold text-red-400 tabular-nums">{formatCurrency(totalCredit)}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase tracking-wider">Fark</p>
              <p className={cn('text-base font-bold tabular-nums', isBalanced ? 'text-emerald-400' : 'text-red-400')}>
                {isBalanced ? '✓ Dengeli' : formatCurrency(diff)}
              </p>
            </div>
          </div>

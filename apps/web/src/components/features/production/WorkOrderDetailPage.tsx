...
import { ArrowLeft, Play, Pause, CheckCircle, XCircle, Package } from 'lucide-react';
...
export function WorkOrderDetailPage({ id }: { id: string }) {
  ...
  if (!wo) return <div className="text-center py-20 text-slate-400">İş emri bulunamadı.</div>;

  const s = STATUS_MAP[wo.status];
  const progress = wo.plannedQty > 0 ? Math.round((wo.producedQty / wo.plannedQty) * 100) : 0;
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            {wo.status === 'PLANNED' && <Button size="sm" onClick={() => changeStatus.mutate({ id, data: { status: 'IN_PROGRESS' } })}><Play className="w-4 h-4" />Başlat</Button>}
            {wo.status === 'IN_PROGRESS' && (
              <>
                <Button variant="ghost" size="sm" onClick={() => changeStatus.mutate({ id, data: { status: 'PAUSED' } })}><Pause className="w-4 h-4" />Duraklat</Button>
                <Button size="sm" onClick={() => changeStatus.mutate({ id, data: { status: 'COMPLETED' } })}><CheckCircle className="w-4 h-4" />Tamamla</Button>
              </>
            )}
            {wo.status === 'PAUSED' && <Button size="sm" onClick={() => changeStatus.mutate({ id, data: { status: 'IN_PROGRESS' } })}><Play className="w-4 h-4" />Devam Et</Button>}
          </div>
        }
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Durum', value: s ? <Badge variant={s.variant}>{s.label}</Badge> : wo.status },
          { label: 'İlerleme', value: `${progress}% (${wo.producedQty} / ${wo.plannedQty})` },
          { label: 'BOM', value: wo.bom ? `${wo.bom.name} v${wo.bom.version}` : '—' },
          { label: 'Tarih', value: wo.startDate ? `${formatDate(wo.startDate)} — ${wo.endDate ? formatDate(wo.endDate) : '?'}` : '—' },
        ].map((item) => (
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 mb-6">
        ...
        <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
          <div className="h-full bg-sky-500 rounded-full transition-all" style={{ width: `${Math.min(progress, 100)}%` }} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {wo.items && wo.items.length > 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-white mb-4">Malzemeler</h3>
                ...
                <div key={item.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-slate-500 font-mono">{item.product?.code}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-sm text-white">{item.consumedQty} / {item.requiredQty}</span>
                    <div className="h-1 w-16 bg-slate-800 rounded-full mt-1">
                      <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${Math.min((item.consumedQty / item.requiredQty) * 100, 100)}%` }} />
                    </div>
                  </div>
                </div>
              ))}
        ...
        {wo.operations && wo.operations.length > 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-white mb-4">Operasyonlar</h3>
            <div className="space-y-3">
                ...
                return (
                  <div key={op.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <span className="block text-xs text-slate-500">{op.workCenter?.name}</span>
                    </div>
          ...
          <div className="space-y-2">
            {wo.history.map((h) => (
              <div key={h.id} className="flex items-center gap-3 text-xs">
                <span className="text-slate-500 w-28 flex-shrink-0">{formatDate(h.createdAt)}</span>
                <span className="text-slate-400">{h.fromStatus ? `${h.fromStatus} → ` : ''}{h.toStatus}</span>

...
export function BOMsPage() {
  ...
  const columns: ColumnDef<BOM>[] = [
    ...
    },
    {
      ...
      align: "center",
      render: (r) => (
        <span className="text-slate-400">{r._count?.items ?? 0}</span>
      ),
    },
    {
      key: "routings",
      ...
      align: "center",
      render: (r) => (
        <span className="text-slate-400">{r._count?.routings ?? 0}</span>
      ),
    },
    {
      key: "workOrders",
      ...
      align: "center",
      render: (r) => (
        <span className="text-slate-400">{r._count?.workOrders ?? 0}</span>
      ),
    },
    {
      key: "isActive",
      ...
      render: (r) => (
        <div className="flex items-center justify-end gap-1">
          <button
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/dashboard/production/boms/${r.id}`);
            }}
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Ürün reçetelerini ve üretim operasyonlarını yönetin."
        action={
      ...
      <DataTable
        ...
        emptyTitle="BOM bulunamadı"
        emptyDescription="Yeni bir ürün ağacı oluşturarak başlayın."
        pagination={
          data
            ? {
                page,
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            label="BOM Adı"
            required
            placeholder="ör. Laptop Pro Reçetesi"
            value={form.name}

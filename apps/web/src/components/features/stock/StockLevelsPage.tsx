...
import type {
  AdvancedStockSuggestion,
  StockLevel,
} from "@/services/stock.service";
import { InventoryTruthGatePanel } from "./InventoryTruthGatePanel";
...
const VELOCITY_TREND_LABEL: Record<
  AdvancedStockSuggestion["salesVelocity"]["trend"],
  string
> = {
...
function StatusBadge({ status }: { status: Exclude<StockStatus, "all"> }) {
  const config = {
    ...
    critical: {
      label: "Kritik",
      className: "border-red-500/25 bg-red-500/8 text-red-300",
      icon: AlertTriangle,
    },
    low: {
      label: "Düşük",
        ...
        <tr key={row} className="border-b border-slate-800/45 last:border-0">
          {Array.from({ length: 7 }).map((_, col) => (
            <td key={col} className="px-4 py-3.5">
              <div
                className="h-3.5 animate-pulse rounded bg-slate-800/75"
                style={{ width: `${42 + ((row + col) % 3) * 18}%` }}
              />
            </td>
          ))}
        </tr>
...
export function StockLevelsPage() {
  ...
  const { data: advancedSuggestions = [], isLoading: loadingSuggestions } =
    useAdvancedStockSuggestions();
  ...
  const visibleSuggestions = useMemo(
    () =>
      warehouseId
        ? advancedSuggestions.filter(
            (suggestion) => suggestion.warehouseId === warehouseId,
          )
        : advancedSuggestions,
  ...
  const filteredLevels = useMemo(() => {
    ...
    return levels
      .filter((level) => {
        const matchesSearch =
          !q ||
          [level.product?.name ?? "", level.product?.code ?? ""].some((value) =>
  ...
  const summary = useMemo(() => {
    const critical = levels.filter(
      (level) => getStatus(level) === "critical",
    ).length;
    ...
    <div className="space-y-4">
      ...
      {isLoading && levels.length === 0 ? (
        ...
        <div className="grid rounded-xl border border-slate-800/80 bg-slate-950/35 sm:grid-cols-4">
          {[
            ...
            {
              label: "Kritik",
              value: numberFormat(summary.critical),
              valueClass:
                summary.critical > 0 ? "text-red-300" : "text-slate-100",
            },
            {
              label: "Stokta Yok",
              value: numberFormat(summary.out),
              valueClass: summary.out > 0 ? "text-red-300" : "text-slate-100",
            },
            {
              label: "Toplam Miktar",
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                {metric.label === "Kritik" && (
                  <span
                    title="Toplam stok minimum stok eşiğinin altına düştüğünde ürün kritik olarak işaretlenir."
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-800/80 bg-slate-950/35 p-3 lg:flex-row lg:items-center">
        ...
        <select
          ...
          className="h-10 w-full rounded-xl border border-slate-700/75 bg-slate-950/35 px-3.5 text-sm text-slate-200 outline-none transition-all duration-150 hover:border-slate-600/80 hover:bg-slate-900/60 focus:border-sky-500/60 focus:ring-2 focus:ring-sky-500/35 lg:w-44"
        >
        ...
        >
          <div>
            <div className="flex items-center gap-2">
              ...
              >
                {visibleSuggestions.length} öneri
              </span>
            </div>
            <p className="mt-0.5 text-xs text-slate-500">
              Mevcut stok, rezervasyon ve tüketim verilerine göre önerilen
              aksiyonlar.
            </p>
          </div>
          <ChevronDown
          ...
          <div>
            {loadingSuggestions ? (
              <div className="divide-y divide-slate-800/65">
                {Array.from({ length: 3 }).map((_, index) => (
                  <div key={index} className="px-4 py-3">
                    ...
                    <div className="mt-2 h-3 w-1/5 animate-pulse rounded bg-slate-800/60" />
                  </div>
                ))}
              </div>
            ) : visibleSuggestions.length === 0 ? (
              <div className="px-4 py-3 text-sm text-slate-500">
                ...
                Tüm ürünler minimum stok seviyelerinin üzerinde.
              </div>
            ) : (
              <div className="divide-y divide-slate-800/65">
                {visibleSuggestions.slice(0, 5).map((suggestion) => (
                  <div
                    ...
                    className={cn(
                      "grid gap-3 border-l-2 px-4 py-3 text-sm md:grid-cols-[minmax(0,1fr)_110px_110px_130px] md:items-center",
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="truncate font-mono text-xs text-slate-500">
                        {suggestion.productCode} · {suggestion.warehouseName}
                      </p>
                    ...
                    <div className="text-right">
                      <p className="font-semibold tabular-nums text-slate-100">
                        {formatQuantity(suggestion.suggestedQuantity)}
                      </p>
                      <p className="text-[11px] text-slate-500">Önerilen</p>
                    </div>
                    <div className="text-right">
                      <p className="font-medium tabular-nums text-slate-300">
                        {formatQuantity(suggestion.pendingReservationQty)}
                      </p>
                      <p className="text-[11px] text-slate-500">
                        {suggestion.reservationCount} rezervasyon
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-medium tabular-nums text-slate-300">
                      ...
                      <p className="text-[11px] text-slate-500">
                        {suggestion.estimatedDaysToStockout === null
                          ? "Tüketim yok"
                          : `${suggestion.estimatedDaysToStockout} gün kapsama`}
                      </p>
                    </div>
                  </div>
                ))}
      ...
      <section>
        <div className="mb-3">
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Depo bazında güncel miktar ve minimum stok karşılaştırması.
          </p>
        </div>
                  ...
                  {[
                    ...
                    <th
                      ...
                      className={cn(
                        ...
                        [2, 3, 4].includes(index) && "text-right",
                        index === 5 && "text-center",
                      )}
                    >
                      {header}
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-12 text-center">
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        Bu filtrelerle eşleşen stok kaydı bulunamadı.
                      </p>
                      {hasFilters && (
                        <div className="mt-4">
                          <Button
                  ...
                  filteredLevels.map((level) => {
                    ...
                    const ratio =
                      min > 0 ? Math.min(1, Math.max(0, quantity / min)) : 1;
                    return (
                      <tr
                        key={level.id}
                        tabIndex={0}
                        onClick={() =>
                          level.product?.id &&
                          router.push(`/dashboard/products/${level.product.id}`)
                        }
                        onKeyDown={(event) => {
                          if (event.key === "Enter" && level.product?.id)
                            router.push(
                              `/dashboard/products/${level.product.id}`,
                            );
                        }}
                      ...
                      >
                        ...
                        <td className="w-10 px-4 py-3.5 text-right">
                          <ChevronRight className="h-4 w-4 text-slate-600 opacity-0 transition-all duration-150 group-hover:translate-x-0.5 group-hover:opacity-100" />
                        </td>
                      </tr>
                    );
                  })
      ...
      {stockAlerts?.summary.alertCount !== undefined &&
        stockAlerts.summary.alertCount > 0 && (
          <p className="text-xs text-slate-500">
            {stockAlerts.summary.alertCount} aktif stok alarmı dashboard izleme
            kapsamında.
          </p>
        )}
    </div>

...
export function StockAlertDashboardCard({
  enabled,
}: StockAlertDashboardCardProps) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-semibold text-slate-100">
              Akilli stok uyarilari
            </h2>
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Tek depo stoklari minimum eşik, bitis riski ve satış hizina göre
              izlenir.
            </p>
            {data?.summary.singleWarehouse && data.summary.warehouseName && (
              <p className="mt-1 text-[11px] text-slate-600">
                Depo: {data.summary.warehouseName}
              </p>
            )}
          </div>
        </div>
                ...
                <div className="mt-3 rounded-lg border border-slate-800 bg-slate-950/50 px-2 py-2">
                  <div className="flex items-center justify-between gap-2 text-[11px]">
                    ...
                    <span className="font-mono text-slate-300">
                      {formatQuantity(item.dailySalesVelocity)}
                    </span>
                  </div>
                  <div className="mt-1 flex items-center justify-between gap-2 text-[11px]">
                    ...
                    <span className="font-mono text-slate-300">
                      {item.estimatedDaysToStockout === null
                        ? "-"
                        : `${item.estimatedDaysToStockout} gün`}
                    </span>
                  </div>
                  <div className="mt-1 flex items-center justify-between gap-2 text-[11px]">

...
function Difference({ diff }: { diff: number }) {
  return (
    <span
      className={cn(
        "inline-flex min-w-[44px] justify-center rounded-md px-2 py-0.5 text-xs font-semibold tabular-nums",
        diff === 0
...
export function StockCountDetailPage({ id }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      surplus,
      match,
      differences: deficit.length + surplus.length,
    };
  }, [items]);
  ...
  if (!count)
    return <div className="text-sm text-slate-400">Sayım bulunamadı.</div>;
  ...
  const filters: {
    ...
    },
    {
      key: "deficit",
      label: "Eksik",
      count: stats.deficit.length,
      className: "text-amber-300",
    },
    {
      key: "surplus",
      label: "Fazla",
      count: stats.surplus.length,
      className: "text-emerald-300",
    },
    {
      key: "match",
      label: "Eşit",
      count: stats.match.length,
    ...
    <div className="space-y-5">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="rounded-lg border border-slate-800 bg-slate-950/40 p-2 text-slate-400 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/40"
            >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-500">
            <span className="flex items-center gap-1.5">
              ...
              {count.warehouse?.name ?? "Depo bilgisi yok"}
            </span>
            <span>{formatDate(count.date)}</span>
            {count.isFinalized && count.finalizedAt && (
              <span>Tamamlanma: {formatDate(count.finalizedAt)}</span>
            )}
          </div>
        </div>
        {!count.isFinalized && (
      ...
      <div className="grid rounded-xl border border-slate-800/80 bg-slate-950/35 sm:grid-cols-4">
        {[
          ...
          {
            label: "Eksik",
            value: stats.deficit.length,
            className:
              stats.deficit.length > 0 ? "text-amber-300" : "text-slate-100",
          },
          {
            label: "Fazla",
            value: stats.surplus.length,
            className:
              stats.surplus.length > 0 ? "text-emerald-300" : "text-slate-100",
          },
          {
            label: "Fark Bulunan",
            value: stats.differences,
            className:
              stats.differences > 0 ? "text-amber-300" : "text-emerald-300",
          },
        ].map((metric) => (
          <div
            key={metric.label}
        ...
        <div className="flex flex-col gap-3 border-b border-slate-800/70 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Sistem stoku ile sayılan miktar karşılaştırması.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {filters.map((item) => (
              <button
                ...
                className={cn(
                  "inline-flex h-8 items-center gap-1.5 rounded-lg border px-3 text-xs font-medium transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/40",
                  filter === item.key
              ...
              >
                ...
                <span
                  className={cn(
                    "tabular-nums",
                    filter === item.key ? "text-sky-200" : item.className,
                  )}
                >
                  {item.count}
                </span>
          ...
          <table className="w-full min-w-[860px] text-sm">
            <thead className="bg-slate-900/95">
              <tr className="border-b border-slate-800/80">
                {["Ürün", "Sistem Stoku", "Sayılan", "Fark", "Durum"].map(
                  (header, index) => (
                    <th
                      key={header}
                      className={cn(
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((item: StockCountItem) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className="border-b border-slate-800/45 transition-colors duration-150 last:border-b-0 hover:bg-sky-500/[0.035]"
                    >
                      ...
                      <td className="px-4 py-3.5 text-right font-semibold tabular-nums text-slate-100">
                        {fmtQty(counted)}
                      </td>
                      <td
                        className={cn(
                      ...
                      <td className="px-4 py-3.5 text-center">
                        {diff < 0 ? (
                          ...
                          <Badge variant="success">Fazla</Badge>
                        ) : (
                          <Badge variant="neutral">Eşit</Badge>
                        )}
                      </td>
                    </tr>
                  );
        ...
        footer={
          <>
            ...
            <Button
              ...
              onClick={() =>
                finalize.mutate(applyAdjustments, {
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3 rounded-xl border border-amber-500/15 bg-amber-500/5 p-3.5">
            ...
            <p className="text-xs leading-relaxed text-slate-400">
              Sayımı tamamladığınızda stok düzeltmeleri uygulanabilir ve işlem
              geçmişe alınır.
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={() => setApplyAdjustments((value) => !value)}
            className={cn(
              "flex w-full items-center gap-3 rounded-xl border p-3 transition-colors duration-150",
              applyAdjustments
          ...
          >
            {applyAdjustments ? (
              ...
              <ToggleLeft className="h-6 w-6 shrink-0 text-slate-600" />
            )}
            <div className="text-left">
              <p
                className={cn(
                  "text-sm font-medium",
                  applyAdjustments ? "text-emerald-300" : "text-slate-400",
                )}
              >
                Stok düzeltmelerini uygula
              </p>
              <p className="mt-0.5 text-xs text-slate-500">
                Farklar otomatik olarak stok hareketine dönüştürülür.
              </p>
            </div>
          </button>
        </div>
...
function MiniStat({
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-950/35 p-3 text-center">
      <div className="mx-auto mb-1 flex justify-center text-slate-500">
        {icon}
      </div>

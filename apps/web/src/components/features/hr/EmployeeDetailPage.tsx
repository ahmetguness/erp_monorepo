...
const LEAVE_STATUS: Record<string, { label: string; variant: 'neutral' | 'success' | 'warning' | 'danger' }> = {
  PENDING: { label: 'Bekliyor', variant: 'warning' }, APPROVED: { label: 'Onaylı', variant: 'success' },
  REJECTED: { label: 'Reddedildi', variant: 'danger' }, CANCELLED: { label: 'İptal', variant: 'neutral' },
};
...
export function EmployeeDetailPage({ id }: { id: string }) {
  ...
  if (!emp) return <div className="text-center py-20 text-slate-400">Personel bulunamadı.</div>;

  return (
    <div>
      <PageHeader title={`${emp.firstName} ${emp.lastName}`} subtitle={[emp.position, emp.department].filter(Boolean).join(' — ')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {emp.leaveRequests && emp.leaveRequests.length > 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-white mb-4">İzin Talepleri</h3>
                ...
                return (
                  <div key={lr.id} className="flex items-center justify-between">
                    <div>
                      ...
                      <span className="block text-xs text-slate-500">{formatDate(lr.startDate)} — {formatDate(lr.endDate)} ({lr.days} gün)</span>
                    </div>
        ...
        {emp.payrolls && emp.payrolls.length > 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-white mb-4">Bordro Geçmişi</h3>
            <div className="space-y-3">
              {emp.payrolls.map((pr) => (
                <div key={pr.id} className="flex items-center justify-between">
                  <div>
                    <span className="text-sm text-sky-400 font-mono">{pr.period}</span>
                    <span className="block text-xs text-slate-500">Brüt: {formatCurrency(pr.grossSalary)}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm text-white font-medium">{formatCurrency(pr.netSalary)}</span>
                    {pr.paidAt ? <Badge variant="success">Ödendi</Badge> : <Badge variant="warning">Bekliyor</Badge>}
                  </div>
                </div>
              ))}
            </div>

...
import { ContactType, DomainEventOutboxStatus, EntityType, InvoiceType, MovementType, PaymentMethod, PermissionAction, Plan, TenantStatus } from '@prisma/client';
...
async function seedTenant(runId: string, suffix: 'a' | 'b') {
  const tenant = await prisma.tenant.create({
    data: {
      ...
      modules: ['contacts', 'invoicing', 'accounting', 'inventory', 'attachments', 'settings'],
    },
  });

...
async function seedExpiredTrialAccount(password: string) {
  const now = Date.now();
  const trialEndsAt = new Date(now - 24 * 60 * 60 * 1000);
  const tenant = await prisma.tenant.create({
    data: {
      slug: "axon-expired-demo",
      companyName: "Axon Süresi Dolmuş Demo Ltd. Şti.",
      taxNumber: "1234567893",
      taxOffice: "Beşiktaş",
      email: "info@expired-demo.axon.test",
      phone: "+90 532 555 0131",
      address: "Demo Caddesi No:15",
      city: "İstanbul",
      country: "TR",
      sector: "E-Ticaret",
      plan: "PROFESSIONAL",
  ...
  const user = await prisma.user.create({
    data: {
      email: "expired@axondemo.com",
      name: "Süresi Dolmuş Demo Sahibi",
      phone: "+90 532 555 0131",
      password,
      isActive: true,
    },
...
async function seedTenant() {
  ...
  const expiredTrialAccount = await seedExpiredTrialAccount(pw);
...
async function seedMasterData(tenantId: string) {
  // Units
  const [unitAdet, unitKg, unitLt, unitMt, unitKutu, unitPaket] =
    await Promise.all([
  ...
  const catElektronik = await prisma.category.create({
    data: { tenantId, name: "Elektronik" },
  });
  const catBilgisayar = await prisma.category.create({
    data: { tenantId, name: "Bilgisayar", parentId: catElektronik.id },
  });
  const catTelefon = await prisma.category.create({
    data: { tenantId, name: "Telefon", parentId: catElektronik.id },
  });
  const catAksesuar = await prisma.category.create({
    data: { tenantId, name: "Aksesuar", parentId: catElektronik.id },
  });
  const catOfis = await prisma.category.create({
    data: { tenantId, name: "Ofis Malzemeleri" },
  });
  const catHammadde = await prisma.category.create({
    data: { tenantId, name: "Hammadde" },
  });
  const catYarimMamul = await prisma.category.create({
    data: { tenantId, name: "Yarı Mamul" },
  });

  // Tax Rates (Standart ve Tevkifatlı)
  const [kdv0, kdv10, kdv20, tevkifat510, tevkifat710] = await Promise.all([
    prisma.taxRate.create({
      data: { tenantId, name: "KDV %0", rate: 0, isWithholding: false },
    }),
    prisma.taxRate.create({
      data: { tenantId, name: "KDV %10", rate: 10, isWithholding: false },
    }),
    prisma.taxRate.create({
      data: { tenantId, name: "KDV %20", rate: 20, isWithholding: false },
    }),
    prisma.taxRate.create({
      data: {
        tenantId,
        name: "KDV Tevkifatı (5/10)",
        rate: 10,
        isWithholding: true,
      },
    }),
  ...
  const accounts = await Promise.all([
    prisma.ledgerAccount.create({
      data: { tenantId, code: "100", name: "Kasa", accountType: "ASSET" },
    }),
    prisma.ledgerAccount.create({
      data: { tenantId, code: "102", name: "Bankalar", accountType: "ASSET" },
    }),
    prisma.ledgerAccount.create({
      data: { tenantId, code: "120", name: "Alıcılar", accountType: "ASSET" },
    }),
    prisma.ledgerAccount.create({
      data: {
        tenantId,
    ...
    prisma.ledgerAccount.create({
      data: { tenantId, code: "500", name: "Sermaye", accountType: "EQUITY" },
    }),
    prisma.ledgerAccount.create({
      data: {
        tenantId,
...
async function seedWarehouses(tenantId: string) {
  ...
  const warehouse2 = await prisma.warehouse.create({
    data: {
      ...
      name: "Üretim Deposu",
      code: "WH02",
      address: "Dudullu OSB Blok B, İstanbul",
    },
  });
...
async function seedProducts(
  ...
  const productDefs = [
    ...
    {
      code: "P006",
      name: "A4 Fotokopi Kağıdı 500 yp",
      unitId: unitAdet.id,
      catId: catOfis.id,
      taxId: kdv10.id,
      buyPrice: 85,
    ...
    {
      code: "HM001",
      name: "Alüminyum Profil 1m",
      unitId: unitKg.id,
      catId: catHammadde.id,
      taxId: kdv20.id,
      buyPrice: 45,
      sellPrice: 0,
      minStock: 100,
    ...
    {
      code: "YM001",
      name: "Laptop Kasası (Yarı Mamul)",
      unitId: unitAdet.id,
      catId: catYarimMamul.id,
      taxId: kdv20.id,
      buyPrice: 850,
      sellPrice: 0,
      minStock: 20,
  ...
  return Promise.all([
    ...
    prisma.contact.create({
      data: {
        ...
        taxOffice: "Şişli",
        email: "satin@teknolojicozmler.com",
        phone: "+90 212 444 0001",
        city: "İstanbul",
        creditLimit: 150000,
        paymentTermDays: 30,
      },
    ...
    prisma.contact.create({
      data: {
        tenantId,
        type: "CUSTOMER",
        name: "Dijital Medya A.Ş.",
        code: "C002",
        taxNumber: "1122334455",
        taxOffice: "Beşiktaş",
        email: "muhasebe@dijitalmedya.com",
        phone: "+90 212 444 0002",
        city: "İstanbul",
        creditLimit: 75000,
        paymentTermDays: 15,
      },
    ...
    prisma.contact.create({
      data: {
        tenantId,
        type: "CUSTOMER",
        name: "Mavi Yazılım Koop.",
        code: "C003",
        taxNumber: "5544332211",
        taxOffice: "Ankara",
        email: "finans@maviyazilim.com",
        phone: "+90 312 444 0003",
        city: "Ankara",
        creditLimit: 50000,
        paymentTermDays: 45,
      },
    ...
    prisma.contact.create({
      data: {
        ...
        taxOffice: "Levent",
        email: "it@yildizholding.com",
        phone: "+90 212 444 0004",
        city: "İstanbul",
        creditLimit: 500000,
        paymentTermDays: 60,
      },
    ...
    prisma.contact.create({
      data: {
        ...
        taxOffice: "Bornova",
        email: "satin@ege.edu.tr",
        phone: "+90 232 444 0005",
        city: "İzmir",
        creditLimit: 200000,
        paymentTermDays: 90,
      },
    ...
    prisma.contact.create({
      data: {
        tenantId,
        type: "SUPPLIER",
        name: "Global Elektronik Dağıtım",
        code: "S001",
        taxNumber: "6677889900",
        taxOffice: "Ümraniye",
        email: "satis@globalelektronik.com",
        phone: "+90 216 555 0010",
        city: "İstanbul",
        paymentTermDays: 60,
      },
    }),
    prisma.contact.create({
      data: {
        tenantId,
        type: "SUPPLIER",
        name: "Ofis Dünyası Toptan",
        code: "S002",
        taxNumber: "1029384756",
        taxOffice: "Bağcılar",
        email: "siparis@ofisdunyasi.com",
    ...
    prisma.contact.create({
      data: {
        tenantId,
        type: "SUPPLIER",
        name: "TechParts İthalat",
        code: "S003",
        taxNumber: "2233445566",
        taxOffice: "Esenyurt",
        email: "import@techparts.com",
        phone: "+90 212 555 0030",
        city: "İstanbul",
        paymentTermDays: 45,
      },
    }),
    ...
    prisma.contact.create({
      data: {
        tenantId,
        type: "BOTH",
        name: "İnovasyon Teknoloji",
        code: "B001",
        taxNumber: "9988776655",
        taxOffice: "Maslak",
        email: "info@inovasyon.tech",
        phone: "+90 212 555 0040",
        city: "İstanbul",
        creditLimit: 100000,
        paymentTermDays: 30,
      },
...
async function seedSales(
      ...
      items: {
        create: [
          ...
          },
          {
            ...
            description: "Kablosuz Mouse Pro",
            quantity: 2,
            unitPrice: 599,
            discount: 5,
            taxRate: 20,
    ...
    data: [
      {
        ...
        orderId: order1.id,
        toStatus: "DRAFT",
        notes: "Sipariş taslağı oluşturuldu",
      },
      {
        tenantId,
        orderId: order1.id,
        ...
        create: [
          {
            ...
            description: "Mekanik Klavye RGB x5",
            quantity: 5,
            unitPrice: 1299,
            discount: 0,
            taxRate: 20,
        ...
        create: [
          {
            ...
            description: "USB-C Hub 7 Port x10",
            quantity: 10,
            unitPrice: 499,
            discount: 0,
            taxRate: 20,
  ...
  const dn1 = await prisma.deliveryNote.create({
    data: {
      ...
      warehouseId: warehouse.id,
      date: d("2026-03-22"),
      shippedAt: d("2026-03-22"),
      deliveredAt: d("2026-03-23"),
        ...
        create: [
          {
            tenantId,
            productId: products[4].id,
            description: "USB-C Hub 7 Port x5",
            orderedQty: 5,
            deliveredQty: 5,
            sortOrder: 0,
          },
  ...
  await prisma.invoiceHistory.createMany({
    data: [
      ...
      },
      {
        ...
        fromStatus: "DRAFT",
        toStatus: "SENT",
        notes: "GİB e-Fatura olarak iletildi",
      },
      {
        tenantId,
        invoiceId: inv1.id,
        fromStatus: "SENT",
        toStatus: "PAID",
        notes: "Garanti BBVA EFT ile tahsil edildi",
      },
    ],
  });
        ...
        create: [
          {
            ...
            taxRateId: kdv20.id,
            description: "Akıllı Telefon X12",
            quantity: 1,
            unitPrice: 16999,
            discount: 0,
            taxAmount: 3399.8,
        ...
        create: [
          {
            ...
            productId: products[0].id,
            taxRateId: kdv20.id,
            withholdingRateId: tevkifat510.id,
            description: 'Laptop Pro 15" x2 (5/10 Tevkifatlı)',
            quantity: 2,
            unitPrice: 24999,
            discount: 0,
            taxAmount: 9999.6,
            withholdingAmount: 4999.8,
            lineTotal: 54998.8,
            sortOrder: 0,
          },
        ],
  ...
  const bankAccountUSD = await prisma.bankAccount.create({
    data: {
      ...
      iban: "TR98 0006 2000 7654 3210 9876 54",
      currencyCode: "USD",
    },
  });
  const cashTRY = await prisma.cashAccount.create({
    data: { tenantId, name: "Merkez Kasa TRY", currencyCode: "TRY" },
  });
  const cashUSD = await prisma.cashAccount.create({
    data: { tenantId, name: "Döviz Kasası USD", currencyCode: "USD" },
  });
  ...
  await prisma.paymentAllocation.create({
    data: { tenantId, paymentId: pay1.id, invoiceId: inv1.id, amount: 31556.4 },
  });
  ...
  const pay2 = await prisma.payment.create({
    data: {
      ...
      amount: 10000,
      method: "BANK_TRANSFER",
      reference: "EFT-2026-003",
      status: "COMPLETED",
      notes: "INV-000002 kısmi tahsilat",
    },
  });
  await prisma.paymentAllocation.create({
    data: { tenantId, paymentId: pay2.id, invoiceId: inv2.id, amount: 10000 },
  });
    ...
    data: [
      {
        ...
        credit: 0,
        balance: 31556.4,
        description: "INV-000001 satış faturası",
        refType: "INVOICE",
        refId: inv1.id,
      },
      {
        ...
        credit: 31556.4,
        balance: 0,
        description: "EFT-2026-001 ödeme",
        refType: "PAYMENT",
        refId: pay1.id,
      },
      {
        ...
        credit: 0,
        balance: 20398.8,
        description: "INV-000002 satış faturası",
        refType: "INVOICE",
        refId: inv2.id,
      },
      {
        ...
        credit: 10000,
        balance: 10398.8,
        description: "EFT-2026-003 kısmi ödeme",
        refType: "PAYMENT",
        refId: pay2.id,
      },
      {
        ...
        credit: 0,
        balance: 5988,
        description: "INV-000003 satış faturası (gecikmiş)",
        refType: "INVOICE",
        refId: inv3.id,
      },
    ],
...
async function seedEDocuments(
  ...
  await prisma.eDocument.create({
    data: {
      ...
      providerMessage:
        "1300: Belge GİB sisteminde başarıyla doğrulandı ve alıcıya iletildi.",
  ...
  await prisma.eDocument.create({
    data: {
      ...
      providerMessage:
        "1163: Gönderilen faturada alıcı VKN/TCKN bilgisi GİB e-Fatura mükellef listesinde bulunamadı.",
      retryCount: 2,
      ...
      responsePayload: {
        errorDetail: "Recipient is not registered in PK registry",
      },
    },
  });
...
async function seedPurchasing(
        ...
        create: [
          {
            ...
            description: "A4 Kağıt x100 paket",
            quantity: 100,
            unitPrice: 85,
          },
        ],
  ...
  await prisma.purchaseOrderHistory.createMany({
    data: [
      ...
      { tenantId, orderId: po1.id, fromStatus: "DRAFT", toStatus: "SENT" },
      {
        ...
        fromStatus: "SENT",
        toStatus: "RECEIVED",
        notes: "3 adet eksiksiz teslim alındı",
      },
    ],
  });
  ...
  await prisma.stockMovement.create({
    data: {
      ...
      refId: po1.id,
      notes: `Satın alma teslimi: ${po1.number}`,
    },
  });
  ...
  await prisma.accountEntry.create({
    data: {
      ...
      credit: 64800,
      balance: -64800,
      description: "INV-000005 alış faturası",
      refType: "INVOICE",
      refId: purchInv.id,
    },
  });
  ...
  await prisma.purchaseRequest.update({
    where: { id: pr1.id },
    data: { status: "ORDERED", purchaseOrderId: po1.id },
  });
...
async function seedAccounting(
  ...
  await prisma.journalEntry.create({
    data: {
      ...
      number: "JE-000001",
      date: d("2026-03-22"),
      description: "INV-000001 satış faturası mahsup kaydı",
      isPosted: true,
      postedAt: d("2026-03-22"),
      lines: {
        create: [
          {
            tenantId,
  ...
  await prisma.journalEntry.create({
    data: {
      ...
      number: "JE-000002",
      date: d("2026-03-25"),
      description: "EFT-2026-001 tahsilat fişi",
      isPosted: true,
      postedAt: d("2026-03-25"),
      lines: {
        create: [
          {
            tenantId,
            ...
            debit: 31556.4,
            credit: 0,
            description: "Garanti Bankası Giriş",
            sortOrder: 0,
          },
          {
            tenantId,
            ...
            debit: 0,
            credit: 31556.4,
            description: "Alıcı Cari Kapatma",
            sortOrder: 1,
          },
        ],
      },
  ...
  await prisma.journalEntry.create({
    data: {
      ...
      number: "JE-000003",
      date: d("2026-04-05"),
      description: "INV-000005 alış faturası kaydı",
      isPosted: true,
      postedAt: d("2026-04-05"),
      lines: {
        create: [
          {
            tenantId,
            ...
            debit: 54000,
            credit: 0,
            description: "Ticari Mallar Giriş",
            sortOrder: 0,
          },
          {
            tenantId,
  ...
  await prisma.journalEntry.create({
    data: {
      ...
      number: "JE-000004",
      date: d("2026-04-30"),
      description: "Nisan 2026 Ofis Kira Gideri",
      isPosted: false,
      lines: {
        create: [
          {
            ...
            debit: 15000,
            credit: 0,
            description: "Genel Yönetim / Kira",
            sortOrder: 0,
          },
          {
            tenantId,
            ...
            debit: 0,
            credit: 15000,
            description: "Garanti Bankası Çıkış",
            sortOrder: 1,
          },
        ],
      },
  ...
  const recon = await prisma.reconciliation.create({
    data: {
      tenantId,
      name: "Mart 2026 Garanti Bankası Mutabakatı",
      date: d("2026-03-31"),
      isFinalized: true,
      finalizedAt: d("2026-04-02"),
    },
  });
    ...
    data: [
      {
        ...
        type: "DEPOSIT",
        amount: 31556.4,
        balanceAfter: 31556.4,
        date: d("2026-03-25"),
        description: "EFT-2026-001 Müşteri Tahsilatı",
        refType: "PAYMENT",
      },
      {
        tenantId,
        ...
        type: "WITHDRAWAL",
        amount: 64800,
        balanceAfter: -33243.6,
        date: d("2026-04-06"),
        description: "PO-000001 Tedarikçi Ödemesi",
        refType: "PAYMENT",
      },
      {
        tenantId,
        ...
        type: "DEPOSIT",
        amount: 10000,
        balanceAfter: -23243.6,
        date: d("2026-04-10"),
        description: "EFT-2026-003 Kısmi Tahsilat",
        refType: "PAYMENT",
      },
    ],
  });
    ...
    data: [
      {
        ...
        amount: 20000,
        currencyCode: "TRY",
        issueDate: d("2026-04-01"),
        dueDate: d("2026-05-01"),
        bankName: "Garanti BBVA",
        status: "PENDING",
        notes: "Müşteri çeki portföyde",
      },
      {
        tenantId,
        type: "CHECK",
      ...
      },
      {
        ...
        amount: 50000,
        currencyCode: "TRY",
        issueDate: d("2026-04-10"),
        dueDate: d("2026-07-10"),
        status: "PENDING",
        notes: "Yıldız Holding vadelendirme senedi",
      },
    ],
  });
}
  ...
  const employees = await Promise.all([
    ...
    prisma.employee.create({
      data: {
        ...
        firstName: "Mustafa",
        lastName: "Arslan",
        email: "mustafa@axondemo.com",
        phone: "+90 532 100 0006",
        position: "Satış Temsilcisi",
        department: "Satış",
        hireDate: d("2023-01-10"),
        salary: 25000,
        isActive: true,
      },
    }),
    prisma.employee.create({
      data: {
        ...
        firstName: "Ayşe",
        lastName: "Koç",
        email: "ayse@axondemo.com",
        phone: "+90 532 100 0007",
        position: "İK Uzmanı",
        department: "İnsan Kaynakları",
        hireDate: d("2023-05-15"),
        salary: 28000,
        isActive: true,
      },
    }),
    ...
    data: [
      {
        ...
        days: 5,
        approvedAt: d("2026-04-10"),
        notes: "Yıllık izin planı",
      },
      {
        tenantId,
        ...
        endDate: d("2026-04-08"),
        days: 2,
        approvedAt: d("2026-04-07"),
        notes: "Raporlu sağlık izni",
      },
      {
        tenantId,
      ...
      },
      {
        ...
        days: 6,
        notes: "Yoğun satış dönemi nedeniyle onaylanmadı",
      },
    ],
  });
  ...
  for (const emp of employees) {
    const payroll = await prisma.payroll.create({
      data: {
        ...
        deductions: Number(emp.salary) * 0.15,
        netSalary: Number(emp.salary) * 0.85,
        paidAt: d("2026-04-30"),
        notes: "Nisan 2026 bordro tahakkuku",
      },
    });
...
async function seedProduction(
  ...
  const [wcMontaj, wcTest, wcPaketleme] = await Promise.all([
    prisma.workCenter.create({
      data: {
        ...
        code: "WC01",
        name: "Montaj Hattı",
        description: "Ürün montaj istasyonu",
        capacity: 8,
        laborRate: 150,
        overheadRate: 50,
        isActive: true,
    ...
    prisma.workCenter.create({
      data: {
        ...
        code: "WC02",
        name: "Test İstasyonu",
        description: "Kalite kontrol ve test",
        capacity: 4,
        laborRate: 200,
        overheadRate: 60,
        isActive: true,
    ...
    prisma.workCenter.create({
      data: {
        ...
        description: "Son paketleme ve etiket",
        capacity: 6,
        laborRate: 120,
        overheadRate: 40,
        isActive: true,
  ...
  const wo1 = await prisma.workOrder.create({
    data: {
      ...
      inputWarehouseId: warehouse2.id,
      outputWarehouseId: warehouse2.id,
      estimatedMaterialCost: 562.0,
      estimatedLaborCost: 350.0,
      estimatedOverheadCost: 120.0,
      actualMaterialCost: 570.0,
      actualLaborCost: 360.0,
      actualOverheadCost: 115.0,
      scrapQty: 1.0,
      scrapCost: 28.0,
      scrapReason: "Montaj esnasında vida yuvası çatlağı",
      items: {
        create: [
          {
            tenantId,
        ...
        create: [
          {
            ...
            stepOrder: 1,
            status: "COMPLETED",
            actualStartAt: d("2026-04-01"),
            actualEndAt: d("2026-04-03"),
            actualSetupTime: 15,
            actualRunTime: 30,
          },
          {
            ...
            stepOrder: 2,
            status: "COMPLETED",
            actualStartAt: d("2026-04-03"),
            actualEndAt: d("2026-04-04"),
            actualSetupTime: 5,
            actualRunTime: 10,
          },
          {
            ...
            stepOrder: 3,
            status: "COMPLETED",
            actualStartAt: d("2026-04-04"),
            actualEndAt: d("2026-04-05"),
            actualSetupTime: 5,
            actualRunTime: 5,
          },
        ],
      ...
      history: {
        create: [
          { tenantId, toStatus: "PLANNED" },
          {
            ...
            notes: "Üretime başlandı",
          },
          {
            tenantId,
            fromStatus: "IN_PROGRESS",
            toStatus: "COMPLETED",
            notes: "20 adet üretildi",
          },
        ],
      },
    },
  ...
  const wo2 = await prisma.workOrder.create({
    data: {
      ...
      inputWarehouseId: warehouse2.id,
      outputWarehouseId: warehouse2.id,
      estimatedMaterialCost: 420.0,
      estimatedLaborCost: 260.0,
      estimatedOverheadCost: 90.0,
      actualMaterialCost: 220.0,
      actualLaborCost: 140.0,
      actualOverheadCost: 50.0,
      items: {
        create: [
          {
            tenantId,
  ...
  await prisma.stockMovement.create({
    data: {
      ...
      refType: "WORK_ORDER",
      refId: wo1.id,
      notes: `İş emri ${wo1.number} üretim çıktısı`,
    },
  });
  await prisma.stockLevel.updateMany({
...
async function seedService(
  ...
  const [asset1, asset2, asset3] = await Promise.all([
    prisma.customerAsset.create({
      data: {
        tenantId,
        contactId: c1.id,
        name: 'Laptop Pro 15" SN:LP001',
        brand: "Axon",
        model: "Pro 15",
        serialNo: "LP-2024-001",
        purchaseDate: d("2024-06-15"),
        warrantyEnd: d("2026-06-15"),
        isActive: true,
      },
    }),
    prisma.customerAsset.create({
      data: {
        tenantId,
        contactId: c2.id,
        name: "Akıllı Telefon X12 SN:AT001",
        brand: "Axon",
        model: "X12",
        serialNo: "AT-2025-001",
        purchaseDate: d("2025-01-10"),
        warrantyEnd: d("2027-01-10"),
        isActive: true,
      },
    }),
    prisma.customerAsset.create({
      data: {
        tenantId,
        contactId: c3.id,
        name: 'Monitor 27" SN:MN001',
        brand: "Axon",
        model: "27K",
        serialNo: "MN-2023-001",
        purchaseDate: d("2023-09-20"),
        warrantyEnd: d("2025-09-20"),
        isActive: true,
      },
    }),
  ]);
  ...
  const sr1 = await prisma.serviceRequest.create({
    data: {
      tenantId,
      contactId: c1.id,
      customerAssetId: asset1.id,
      number: "SR-000001",
      status: "COMPLETED",
      priority: "HIGH",
      subject: "Laptop ekran piksel hatası",
      description:
        "Ekranın sol alt köşesinde ölü piksel var, garanti kapsamında değişim yapıldı.",
      warrantyEnd: asset1.warrantyEnd,
      closedAt: d("2026-04-10"),
      items: {
        create: [
          {
            tenantId,
            productId: products[0].id,
            description: "Ekran değişimi parça & işçilik",
            quantity: 1,
            unitPrice: 0,
            lineTotal: 0,
          },
        ...
        create: [
          {
            tenantId,
            activityType: "NOTE",
            notes: "Cihaz kargo ile teslim alındı",
            createdAt: d("2026-04-05"),
          },
          {
            tenantId,
            activityType: "STATUS_CHANGE",
            notes: "OPEN → IN_PROGRESS",
          },
          {
            tenantId,
            activityType: "CALL",
            notes: "Müşteriye parça temin süresi iletildi",
          },
          {
            tenantId,
            activityType: "STATUS_CHANGE",
            notes: "IN_PROGRESS → COMPLETED",
          },
        ],
      },
      history: {
        create: [
          { tenantId, toStatus: "OPEN" },
          {
            ...
            fromStatus: "OPEN",
            toStatus: "IN_PROGRESS",
            notes: "Teknik servis incelemesi",
          },
          {
            tenantId,
            fromStatus: "IN_PROGRESS",
            toStatus: "COMPLETED",
            notes: "Değişim tamamlandı",
          },
        ],
      },
    },
...
async function seedMarketplace(tenantId: string, products: { id: string }[]) {
  ...
  const hb = await prisma.marketplaceIntegration.create({
    data: {
      tenantId,
      channel: "HEPSIBURADA",
      name: "Hepsiburada Resmi Mağaza",
      apiKey: "hb-api-key",
      apiSecret: "hb-secret",
      storeId: "hb-98765",
      isActive: true,
      lastSyncAt: d("2026-05-04"),
    },
  });
  ...
  await prisma.marketplaceOrder.create({
    data: {
      tenantId,
      integrationId: hb.id,
      externalId: "HB-ORD-101",
      channel: "HEPSIBURADA",
      status: "SHIPPED",
      customerName: "Ayşe Güler",
      customerEmail: "ayse.guler@email.com",
      shippingAddress: "Çankaya, Ankara",
      totalAmount: 1198,
      orderDate: d("2026-05-02"),
      items: {
        create: [
          {
            tenantId,
            externalProductId: "HB-MS001",
            productId: products[2].id,
            name: "Kablosuz Mouse Pro x2",
            quantity: 2,
            unitPrice: 599,
            lineTotal: 1198,
          },
  ...
  await prisma.marketplaceListing.create({
    data: {
      tenantId,
      integrationId: amz.id,
      productId: products[4].id,
      externalId: "AMZ-HB001",
      externalSku: "AXON-HB-001",
      price: 499,
      stock: 50,
      isActive: true,
      lastSyncAt: d("2026-05-04"),
    ...
    data: [
      {
        ...
        status: "DONE",
        processedCount: 4,
        startedAt: d("2026-05-03"),
        finishedAt: d("2026-05-03"),
      },
      {
        tenantId,
        integrationId: hb.id,
        ...
        status: "DONE",
        processedCount: 15,
        startedAt: d("2026-05-04"),
        finishedAt: d("2026-05-04"),
      },
    ],
  });
...
async function seedRolesAndApprovals(
  ...
  const roleSales = await prisma.role.create({
    data: {
      ...
      description: "Satış ve CRM erişimi",
      isSystem: false,
      permissions: {
        create: [
  ...
  const roleAccounting = await prisma.role.create({
    data: {
      ...
      description: "Muhasebe ve finans tam erişim",
      isSystem: false,
      permissions: {
        create: [
  ...
  const roleWarehouse = await prisma.role.create({
    data: {
      tenantId,
      name: "Depo Sorumlusu",
      description: "Depo ve stok yönetimi",
      isSystem: false,
      permissions: {
        create: [
  ...
  await prisma.tenantUser.updateMany({
    where: { tenantId, userId: userSales.id },
    data: { roleId: roleSales.id },
  });
  await prisma.tenantUser.updateMany({
    where: { tenantId, userId: userAccounting.id },
    data: { roleId: roleAccounting.id },
  });
  await prisma.tenantUser.updateMany({
    where: { tenantId, userId: userWarehouse.id },
    data: { roleId: roleWarehouse.id },
  });
        ...
        create: [
          {
            ...
            approverRoleId: roleSales.id,
            isRequired: true,
          },
          {
            stepOrder: 2,
            name: "Finans Onayı",
            approverRoleId: roleAccounting.id,
            isRequired: true,
          },
        ],
      },
  ...
  const req1 = await prisma.approvalRequest.create({
    data: {
      tenantId,
      flowId: flowPurchase.id,
      entityType: "PURCHASE_ORDER",
      entityId: purchasing.pr1.id,
      status: "APPROVED",
      currentStep: 2,
      requestedBy: userWarehouse.name,
      notes: "Acil üretim hammadde ihtiyacı için onay talebi",
      actions: {
        create: [
          {
            stepId: flowPurchase.steps[0].id,
            actionType: "APPROVE",
            actorId: userSales.id,
            notes: "Departman bütçesi uygun, onaylandı.",
          },
          {
            stepId: flowPurchase.steps[1].id,
            actionType: "APPROVE",
            actorId: userAccounting.id,
            notes: "Ödeme vadesi onaylandı.",
          },
        ],
      },
    },
  ...
  await prisma.approvalRequest.create({
    data: {
      ...
      flowId: flowPurchase.id,
      entityType: "PURCHASE_ORDER",
      entityId: purchasing.pr2.id,
      status: "PENDING",
      currentStep: 1,
      requestedBy: userSales.name,
...
async function seedCollaborationAndWorkflow(
    ...
    data: [
      {
        ...
        type: "COMMENT",
        content: "Müşteri faturayı EFT ile ödedi, muhasebe teyit etti.",
        mentionIds: [userAccounting.id],
        createdById: userSales.id,
        createdAt: d("2026-03-25T11:00:00"),
      },
      {
        tenantId,
        entityType: "INVOICE",
        ...
        type: "DECISION",
        content:
          "Vade 60 gün aşıldığı için hukuki takip öncesi son hatırlatma kararı alındı.",
        mentionIds: [userAdmin.id, userAccounting.id],
        createdById: userAccounting.id,
        createdAt: d("2026-04-02T14:30:00"),
      },
      {
        tenantId,
        ...
        type: "COMMENT",
        content:
          "Yurtiçi Kargo takip numarası sisteme girildi, sevkiyat tamamlandı.",
        mentionIds: [userSales.id],
        createdById: userWarehouse.id,
        createdAt: d("2026-03-22T16:00:00"),
      },
    ],
  });
  ...
  await prisma.task.createMany({
    data: [
      {
        tenantId,
        title: "Mavi Yazılım (INV-000003) Tahsilat Takibi",
        detail:
          "Gecikmiş fatura için şirket yetkilisi aranarak mutabakat sağlanacak.",
        type: "COLLECTION",
        priority: "HIGH",
        status: "IN_PROGRESS",
        module: "invoicing",
        entityType: "INVOICE",
        entityId: invoices[2].id,
        assignedToId: userAccounting.id,
        createdById: userAdmin.id,
        dueAt: d("2026-05-15"),
      },
      {
        tenantId,
        title: "A4 Kağıt Minimum Stok Seviyesi İncelemesi",
        detail:
          "Depoda 3 paket kağıt kaldı, satın alma siparişinin teslimatı takip edilecek.",
        type: "CHECK",
        priority: "MEDIUM",
        status: "TODO",
        module: "inventory",
        assignedToId: userWarehouse.id,
        createdById: userAdmin.id,
        dueAt: d("2026-05-10"),
      },
      {
        tenantId,
        title: "SR-000002 Telefon Batarya Değişimi Teknik Testi",
        ...
        entityId: service.sr2.id,
        assignedToId: userSales.id,
        createdById: userAdmin.id,
        dueAt: d("2026-05-12"),
      },
      {
        tenantId,
        title: "Nisan Ayı KDV Beyannamesi Onayı",
        detail:
          "Vergi dairesi öncesi beyanname kontrolü ve onay süreci tamamlandı.",
        type: "APPROVAL",
        priority: "CRITICAL",
        status: "DONE",
        module: "accounting",
    ...
    data: [
      {
        ...
        direction: "OUTBOUND",
        status: "SENT",
        from: "muhasebe@axondemo.com",
        to: ["satin@teknolojicozmler.com"],
        cc: ["satis@axondemo.com"],
        ...
        attachmentCount: 1,
        sentById: userSales.id,
        sentAt: d("2026-03-22T10:30:00"),
      },
      {
        tenantId,
        direction: "OUTBOUND",
        status: "SENT",
        from: "lojistik@axondemo.com",
        to: ["satin@teknolojicozmler.com"],
        ...
        attachmentCount: 0,
        sentById: userWarehouse.id,
        sentAt: d("2026-03-22T17:00:00"),
      },
      {
        tenantId,
        direction: "INBOUND",
        status: "SENT",
        from: "satin@teknolojicozmler.com",
    ...
    data: [
      {
        ...
        message: "SIP-000004 numaralı sipariş oluşturuldu.",
        module: "invoicing",
        entityType: "SALES_ORDER",
        status: "UNREAD",
      ...
      },
      {
        ...
        message: "INV-000002 faturasının vadesi 3 gün sonra.",
        module: "invoicing",
        entityType: "INVOICE",
        status: "UNREAD",
        createdAt: d("2026-04-09"),
      },
      {
        tenantId,
        userId: userAdmin.id,
        title: "Gecikmiş Fatura Uyarısı",
        message: "INV-000003 faturası 60 gün gecikmiş.",
        module: "invoicing",
        entityType: "INVOICE",
        status: "READ",
      ...
      },
      {
        ...
        message: "P006 (A4 Kağıt) minimum stok seviyesinin altında.",
        module: "inventory",
        entityType: "PRODUCT",
        status: "UNREAD",
      ...
      },
      {
        ...
        message: "WO-000001 iş emri tamamlandı. 20 adet üretildi.",
        module: "production",
      ...
      },
      {
        ...
        message: "SR-000001 servis talebi başarıyla tamamlandı.",
        module: "service",
        entityType: "SERVICE_REQUEST",
        status: "ARCHIVED",
        createdAt: d("2026-04-10"),
      },
      {
        tenantId,
        userId: userAdmin.id,
        title: "Hepsiburada Senkronizasyonu",
        message: "HB-ORD-101 numaralı sipariş sisteme aktarıldı.",
        module: "marketplace",
        status: "READ",
        createdAt: d("2026-05-02"),
        readAt: d("2026-05-02"),
      },
      {
        tenantId,
        ...
        message: "PR-000002 satın alma talebi onayınızı bekliyor.",
        module: "purchasing",
        entityType: "PURCHASE_ORDER",
        status: "UNREAD",
        createdAt: d("2026-04-10"),
...
async function seedAutomations(tenantId: string) {
  const rule1 = await prisma.automationRule.create({
    data: {
      ...
      description:
        "Ürün stoku asgari seviyenin altına indiğinde otomatik depo görevi açar.",
      trigger: "LOW_STOCK",
      action: "CREATE_TASK",
      module: "inventory",
  ...
  const rule2 = await prisma.automationRule.create({
    data: {
      ...
      description:
        "Vadesi 7 gün geçen faturalar için otomatik tahsilat hatırlatma e-postası hazırlar.",
  ...
  const rule3 = await prisma.automationRule.create({
    data: {
      ...
      description:
        "100.000 TL üzeri satış siparişlerinde otomatik onay talebi başlatır.",
    ...
    data: [
      {
        ...
        output: {
          ...
          message: "Depo görevi başarıyla oluşturuldu",
        },
        startedAt: d("2026-05-01T08:00:00"),
        completedAt: d("2026-05-01T08:00:02"),
      },
      {
        tenantId,
        ruleId: rule2.id,
        ...
        startedAt: d("2026-04-15T09:00:00"),
        completedAt: d("2026-04-15T09:00:01"),
      },
    ],
  });
    ...
    data: [
      {
        tenantId,
        entityType: "INVOICE",
        entityId: sales.invoices[0].id,
        fileName: "INV-000001-Fatura.pdf",
        storagePath: "tenants/axon-demo/invoices/INV-000001.pdf",
      ...
      },
      {
        ...
        fileName: "Laptop-Pro-15-Teknik-Sartname.pdf",
        storagePath: "tenants/axon-demo/products/laptop-spec.pdf",
        mimeType: "application/pdf",
        fileSize: 1250000,
        category: "SPEC_SHEET",
        documentKind: "MANUAL",
        confidentiality: "PUBLIC",
        tags: ["katalog", "bilgisayar"],
        uploadedById: userId,
      },
      {
        tenantId,
        entityType: "CUSTOMER_ASSET",
        entityId: service.asset1.id,
        fileName: "Garanti-Belgesi-AxonPro15.pdf",
        storagePath: "tenants/axon-demo/assets/warranty-lp001.pdf",
        mimeType: "application/pdf",
        fileSize: 320000,
      ...
      },
      {
        tenantId,
        entityType: "SERVICE_REQUEST",
        entityId: service.sr1.id,
        fileName: "ekran-ariza-goruntusu.jpg",
        storagePath: "tenants/axon-demo/service/sr-001-defect.jpg",
        mimeType: "image/jpeg",
        fileSize: 412000,
        category: "DEFECT_PHOTO",
        documentKind: "PHOTO",
        confidentiality: "INTERNAL",
        tags: ["servis", "ariza"],
        uploadedById: userId,
...
async function seedSavedViewsAndOutbox(
  ...
  await prisma.savedView.createMany({
    data: [
      {
        tenantId,
        name: "Vadesi Geçmiş Satış Faturaları",
        module: "invoicing",
        listKey: "invoices",
        ...
        state: {
          ...
          sortBy: "dueDate",
          sortDir: "asc",
          columns: ["number", "contact", "dueDate", "totalGross"],
        },
      },
      {
        tenantId,
        name: "Kritik Stok Uyarısı Veren Ürünler",
        module: "inventory",
        listKey: "products",
        scope: SavedViewScope.TENANT,
        isDefault: true,
      ...
      },
      {
        tenantId,
        userId,
        name: "Yüksek Kredili VIP Müşterilerim",
        module: "contacts",
        listKey: "contacts",
    ...
    data: [
      {
        ...
        idempotencyKey: "outbox-inv-001",
        entityType: "INVOICE",
        entityId: sales.invoices[0].id,
        payload: { invoiceNumber: "INV-000001", amount: 31556.4 },
        context: { actorId: userId },
        status: DomainEventOutboxStatus.PROCESSED,
        processedAt: d("2026-03-22T10:16:00"),
      },
      {
        tenantId,
        name: "payment.completed",
        source: "finance.payment.service",
        idempotencyKey: "outbox-pay-001",
        entityType: "INVOICE",
        entityId: sales.invoices[0].id,
        payload: { paymentRef: "EFT-2026-001", amount: 31556.4 },
        context: { actorId: userId },
        status: DomainEventOutboxStatus.PROCESSED,
        processedAt: d("2026-03-25T11:05:00"),
      },
      {
        tenantId,
        name: "delivery_note.shipped",
        source: "logistics.delivery.service",
        idempotencyKey: "outbox-dn-001",
        entityType: "DELIVERY_NOTE",
        entityId: sales.deliveryNotes[0].id,
        payload: { number: "DN-000001", carrier: "Yurtiçi Kargo" },
        context: { actorId: userId },
        status: DomainEventOutboxStatus.PROCESSED,
        processedAt: d("2026-03-22T17:01:00"),
      },
      {
        tenantId,
        name: "edocument.retry_requested",
        source: "edocument.automation.service",
        idempotencyKey: "outbox-edoc-retry-001",
        entityType: "INVOICE",
        entityId: sales.invoices[2].id,
        payload: {
          edocumentId: "edoc-err-01",
          reason: "GİB VKN düzeltmesi sonrası tekrar gönderim",
        },
        context: {},
        status: DomainEventOutboxStatus.PENDING,
        attempts: 1,
        nextRetryAt: d("2026-05-10T10:00:00"),
      },
    ],
  });
}
...
async function seedBilling(
  ...
  await prisma.billingSubscription.create({
    data: {
      tenantId,
      provider: "IYZICO",
      providerCustomerId: "cus_ent_001",
      state: "ACTIVE",
      monthlyAmount: 14999.0,
      currency: "TRY",
    },
  });
  ...
  await prisma.billingSubscription.create({
    data: {
      ...
      provider: "STRIPE",
      providerCustomerId: "cus_pro_002",
      state: "ACTIVE",
      monthlyAmount: 4999.0,
      currency: "TRY",
    },
  });
  ...
  await prisma.billingInvoice.createMany({
    data: [
      {
        tenantId,
        providerInvoiceId: "inv_sub_2026_01",
        amount: 14999.0,
        currency: "TRY",
        status: "PAID",
        dueAt: d("2026-01-31"),
      ...
      {
        tenantId,
        providerInvoiceId: "inv_sub_2026_02",
        amount: 14999.0,
        currency: "TRY",
        status: "PAID",
        dueAt: d("2026-02-28"),
      ...
      {
        tenantId,
        providerInvoiceId: "inv_sub_2026_03",
        amount: 14999.0,
        currency: "TRY",
        status: "PAID",
        dueAt: d("2026-03-31"),
      ...
      {
        tenantId,
        providerInvoiceId: "inv_sub_2026_04",
        amount: 14999.0,
        currency: "TRY",
        status: "PAID",
        dueAt: d("2026-04-30"),
  ...
  await prisma.customPriceRequest.create({
    data: {
      ...
      expiresAt: d("2026-12-31"),
      reason:
        "100+ kullanıcı taahhüdü kapsamında yıllık peşin ödeme özel fiyat teklifi.",
      state: "APPLIED",
...
async function seedSupportTickets(
  ...
  const ticket1 = await prisma.platformSupportTicket.create({
    data: {
      ...
      description:
        "Müşterimize e-fatura keserken 1163 VKN bulunamadı hatası almaktayız. Test ortamında mükellef doğrulaması yapılması gerekiyor.",
      category: PlatformTicketCategory.TECHNICAL,
      priority: PlatformTicketPriority.HIGH,
      status: PlatformTicketStatus.IN_PROGRESS,
      messages: {
        create: [
          {
            senderType: PlatformTicketSender.TENANT_USER,
            authorUserId: userId,
            message:
              "Merhaba, e-fatura modülünde alıcı carinin VKN bilgisi doğru olmasına rağmen GİB tarafı 1163 hatası döndürüyor.",
            createdAt: d("2026-04-12T10:00:00"),
          },
          {
            senderType: PlatformTicketSender.ADMIN_USER,
            authorAdminId: adminUserId,
            message:
              "Merhabalar Ahmet Bey, entegratör loglarını inceledik. Alıcı test ortamında kayıtlı değil, gerçek ortam ile test ortamı endpoint ayrımı güncellendi.",
          ...
          },
          {
            ...
            isInternal: true,
            message:
              "GİB test API servisindeki sertifika yenilendi. Kiracının endpoint parametreleri doğrulandı.",
  ...
  const ticket2 = await prisma.platformSupportTicket.create({
    data: {
      ...
      description:
        "Hepsiburada satıcı paneli üzerinden oluşturulan yeni API anahtarı sisteme bağlandı.",
      category: PlatformTicketCategory.FEATURE_REQUEST,
      priority: PlatformTicketPriority.MEDIUM,
      status: PlatformTicketStatus.RESOLVED,
      resolvedAt: d("2026-04-15T15:00:00"),
      messages: {
        create: [
          {
            senderType: PlatformTicketSender.TENANT_USER,
            authorUserId: userId,
            message:
              "Hepsiburada entegrasyonu başarılı çalışıyor, teşekkür ederiz.",
          },
        ],
      },
    },
...
async function seedEnterpriseGovernance(
  ...
  await prisma.apiKey.createMany({
    data: [
      {
        tenantId,
        name: "B2B ERP Entegrasyon Servisi",
        keyPrefix: "sk_live_b2b_",
        keyHash: await hash("sk_live_b2b_secret_key_12345"),
        scopes: ["invoices:read", "orders:create", "inventory:read"],
        isActive: true,
        lastUsedAt: d("2026-05-04T12:00:00"),
      },
      {
        tenantId,
        name: "WMS El Terminali Entegrasyonu",
        keyPrefix: "sk_live_wms_",
        keyHash: await hash("sk_live_wms_secret_key_67890"),
        scopes: ["inventory:read", "inventory:write"],
        isActive: true,
        lastUsedAt: d("2026-05-04T09:30:00"),
      },
    ],
  });
  ...
  await prisma.invitation.createMany({
    data: [
      {
        tenantId,
        email: "yeni.muhasebe@axondemo.com",
        status: "PENDING",
        tokenHash: "inv_hash_001_pending",
        expiresAt: d("2026-06-01"),
        invitedBy: userSalesId,
      },
      {
        tenantId,
        email: "zeynep@axondemo.com",
        status: "ACCEPTED",
        tokenHash: "inv_hash_002_accepted",
        expiresAt: d("2026-04-01"),
        acceptedAt: d("2026-03-01"),
        invitedBy: userSalesId,
      },
    ],
  });
  ...
  await prisma.tenantFeatureOverride.createMany({
    data: [
      {
        tenantId,
        featureKey: FeatureKey.MAX_PRODUCTS,
        value: "unlimited",
        isEnabled: true,
        reason:
          "Enterprise yıllık sözleşme ek protokolü ile ürün limiti sınırsız yapıldı.",
      },
      {
        tenantId,
    ...
    data: [
      {
        ...
        newValues: { number: "INV-000001", totalGross: 31556.4 },
        ipAddress: "192.168.1.45",
        userAgent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
        createdAt: d("2026-03-22T10:00:00"),
      },
      {
        tenantId,
        userId: userSalesId,
        ...
        ipAddress: "192.168.1.45",
        createdAt: d("2026-03-20T14:15:00"),
      },
      {
        tenantId,
        adminId: adminUserId,
        ...
        reason: "Plan ve modül erişimi güncellendi",
        newValues: { plan: "ENTERPRISE", maxUsers: 100 },
        createdAt: d("2026-01-01T09:00:00"),
      },
    ],
  });
    ...
    data: [
      {
        tenantId,
        userId: userSalesId,
        requestType: "MAIL_DRAFT",
        promptVersion: "1.0",
        model: "gemini-1.5-pro",
        entityType: EntityType.INVOICE,
        entityId: "INV-000003",
        inputSummary:
          "Vadesi 15 gün geçmiş fatura için kibar ama kararlı tahsilat hatırlatma e-postası taslağı oluştur",
        outputSummary:
          "Sayın Yetkili, INV-000003 numaralı faturanızın ödeme vadesi geçmiştir...",
        status: "SUCCEEDED",
        tokenPrompt: 380,
        tokenCompletion: 140,
        tokenTotal: 520,
        createdAt: d("2026-04-15T09:00:00"),
        completedAt: d("2026-04-15T09:00:02"),
      },
      {
        tenantId,
        userId: userSalesId,
        requestType: "SMART_FORM",
        promptVersion: "1.0",
        model: "gemini-1.5-flash",
        entityType: EntityType.PRODUCT,
        entityId: "P001",
        inputSummary:
          "Laptop teknik özellik metninden kategori, birim ve vergi oranı öner",
        outputSummary: "Kategori: Bilgisayar, KDV: %20, Birim: Adet",
        status: "SUCCEEDED",
        tokenPrompt: 210,
        tokenCompletion: 45,
        tokenTotal: 255,
        createdAt: d("2026-03-10T11:00:00"),
        completedAt: d("2026-03-10T11:00:01"),
      },
    ],
  });
  ...
  await prisma.supportSession.create({
    data: {
      tenantId,
      adminId: adminUserId,
      adminSessionId: "sess-admin-01",
      targetUserId: userSalesId,
      reason: "GİB bağlantı test parametrelerinin canlı incelenmesi",
      ticketId: "TKT-2026-001",
      scopes: ["invoicing:read", "settings:read"],
      expiresAt: d("2026-05-15"),
      approvedAt: d("2026-05-04"),
    },
  });
}
...
async function seedPlatformHealth(tenantId: string, adminUserId: string) {
    ...
    data: [
      {
        ...
        scope: "platform",
        scopeId: "global",
        metricKey: "http_request_success_ratio",
        targetPercentage: 99.9,
        windowDays: 30,
        owner: "SRE Ekibi",
        runbookUrl: "https://wiki.axonerp.com/runbooks/slo-uptime",
        notificationChannel: "#alerts-slo",
      },
      {
        id: "slo-api-latency",
        ...
        scope: "platform",
        scopeId: "global",
        metricKey: "http_request_duration_p95",
        targetPercentage: 98.5,
        windowDays: 7,
        owner: "Backend Ekibi",
        runbookUrl: "https://wiki.axonerp.com/runbooks/slo-latency",
        notificationChannel: "#alerts-perf",
      },
    ],
  });
    ...
    data: [
      {
        ...
        scopeId: tenantId,
        value: 142.5,
        bucketAt: new Date(now.getTime() - 3600000),
      },
      {
        metricKey: "api_latency_p95",
        scope: "tenant",
    ...
    data: [
      {
        ...
        threshold: 80.0,
        owner: "SRE Ekibi",
        runbookUrl: "https://wiki.axonerp.com/runbooks/mem-spike",
        notificationChannel: "#alerts",
        openedAt: d("2026-05-02T04:10:00"),
        resolvedAt: d("2026-05-02T04:35:00"),
      },
      {
        fingerprint: "fp-sync-hb-01",
        metricKey: "sync_delay_seconds",
        scope: "marketplace",
        scopeId: "hepsiburada",
        severity: "LOW",
        status: "SILENCED",
        ...
        runbookUrl: "https://wiki.axonerp.com/runbooks/marketplace-lag",
        notificationChannel: "#marketplace-ops",
        silencedUntil: d("2026-05-10"),
        silenceReason: "Hepsiburada planlı sunucu bakımı",
      },
    ],
  });
    ...
    data: [
      {
        ...
        description: "GİB E-Fatura UBL-TR 1.2.1 şema güncellemesi",
        deployedAt: d("2026-04-25"),
      ...
      },
      {
        ...
        version: "v2.6.0",
        environment: "production",
        description: "Performans iyileştirmeleri ve yeni otomasyon motoru",
        deployedAt: d("2026-05-01"),
        createdById: adminUserId,
      },
    ],
  });
  ...
  await prisma.observabilityLogEntry.createMany({
    data: [
      {
        fingerprint: "log-err-01",
        level: "ERROR",
        service: "edocument-service",
        message: "GİB PK web service validation error 1163",
        occurredAt: d("2026-04-12T10:00:00"),
      },
      {
        fingerprint: "log-info-01",
        level: "INFO",
        service: "billing-cron",
        message:
          "Aylık kurumsal abonelik faturalandırması başarıyla tamamlandı",
        occurredAt: d("2026-05-01T00:01:00"),
      },
    ],
  });
  ...
  await prisma.platformSecurityFinding.createMany({
    data: [
      {
        key: "SEC-TLS-1.0-AUDIT",
        title: "TLS 1.0 ve 1.1 Protokol Desteği Kapatıldı",
        category: "NETWORK",
        severity: "LOW",
        status: "RESOLVED",
        verificationStatus: "VERIFIED",
        owner: "DevSecOps Ekibi",
        remediation:
          "Yalnızca TLS 1.2 ve 1.3 şifreleme süitleri aktif tutuldu.",
        evidence: { tlsVersion: "1.3", cipher: "TLS_AES_256_GCM_SHA384" },
      },
      {
        key: "SEC-API-RATE-LIMIT",
        title: "Webhook Endpoint Hız Sınırı Doğrulaması",
        category: "APPLICATION",
        severity: "MEDIUM",
        status: "OPEN",
        verificationStatus: "IN_REVIEW",
        owner: "Güvenlik Ekibi",
        remediation:
          "Dakikada maksimum 120 istek sınırı ve IP bazlı throttling kontrolü.",
  ...
  const incident = await prisma.platformIncident.create({
    data: {
      ...
      summary:
        "Trendyol API sunucularındaki bakım nedeniyle 45 dakika boyunca sipariş çekme istekleri zaman aşımına uğradı.",
      severity: "SEV-3",
      status: "RESOLVED",
      owner: "Operasyon Nöbetçisi",
      runbookUrl: "https://wiki.axonerp.com/runbooks/marketplace-outage",
      createdById: adminUserId,
      resolvedAt: d("2026-05-03T15:00:00"),
      tenants: { create: [{ tenantId }] },
      timeline: {
        create: [
          {
            type: "DETECTED",
            message: "Webhook zaman aşımı uyarıları tetiklendi",
            createdById: adminUserId,
            createdAt: d("2026-05-03T14:15:00"),
          },
          {
            type: "MITIGATED",
            message: "Kuyruk tekrar deneme mekanizması devreye alındı",
            createdById: adminUserId,
            createdAt: d("2026-05-03T14:45:00"),
          },
          {
            type: "RESOLVED",
            message: "Tüm bekleyen siparişler başarıyla içeri aktarıldı",
            createdById: adminUserId,
            createdAt: d("2026-05-03T15:00:00"),
          },
        ],
      },
      communications: {
        create: [
          {
            message:
              "Trendyol entegrasyonundaki geçici gecikme giderilmiş olup siparişleriniz güncellenmiştir.",
            status: "PUBLISHED",
            recipientCount: 150,
            requestedById: adminUserId,
            publishedAt: d("2026-05-03T15:05:00"),
          },
        ],
      },
    },
  ...
  const backup = await prisma.platformBackupEvidence.create({
    data: {
      providerRef: "bak-prod-daily-20260504",
      status: "VERIFIED",
      sizeBytes: BigInt(2450000000),
      encrypted: true,
      encryptionKeyRef: "kms/prod/backup-key-01",
      region: "eu-central-1",
      startedAt: d("2026-05-04T02:00:00"),
      completedAt: d("2026-05-04T02:18:00"),
  ...
  await prisma.platformRestoreDrill.create({
    data: {
      ...
      runbookUrl: "https://wiki.axonerp.com/runbooks/dr-drill",
      approvalId: "APPR-DR-2026-Q1",
      requestedById: adminUserId,
      startedAt: d("2026-05-04T10:00:00"),
      completedAt: d("2026-05-04T10:42:00"),
      actualRpoMinutes: 14,
      actualRtoMinutes: 42,
      notes:
        "Felaket kurtarma simülasyonu başarıyla tamamlandı. RPO/RTO hedefleri sağlandı.",
    },
  });
  ...
  await prisma.dataSubjectRequest.create({
    data: {
      tenantId,
      subjectEmail: "eski.calisan@email.com",
      type: "EXPORT",
      status: "APPROVED",
      scope: { modules: ["hr", "attendance"] },
      reason: "KVKK 11. madde uyarınca kişisel verilerin dökümü talebi",
      ticketId: "DSR-2026-001",
      requestedById: adminUserId,
  ...
  await prisma.privacyLegalHold.create({
    data: {
      tenantId,
      reason: "2025 Mali Yılı Gelir İdaresi Başkanlığı Vergi İncelemesi",
      active: true,
      createdById: adminUserId,
    },
  });
  ...
  await prisma.featureRollout.create({
    data: {
      plan: "ENTERPRISE",
      featureKey: FeatureKey.CASHFLOW_FORECAST,
      version: 1,
      environment: "production",
      stage: "BETA",
      status: "ACTIVE",
      ...
      startsAt: d("2026-05-01"),
      errorThresholdPct: 2.0,
      reason: "Nakit Akış Tahminleme Modülü Kademeli Dağıtımı",
      createdById: adminUserId,
      activatedById: adminUserId,
      activatedAt: d("2026-05-01"),
    },
  });
  ...
  const demoReq1 = await prisma.demoRequest.create({
    data: {
      fullName: "Canberk Aktaş",
      companyName: "Aktaş Lojistik A.Ş.",
      email: "canberk@aktaslogistics.com",
      phone: "+90 532 999 0001",
      plan: "ENTERPRISE",
      status: DemoRequestStatus.APPROVED,
      processedBy: "admin@axonerp.com",
      processedAt: d("2026-05-02"),
  ...
  await prisma.demoRequest.create({
    data: {
      ...
      companyName: "Çetinkaya Perakende",
      email: "seda@cetinkaya.com",
      phone: "+90 532 999 0002",
      plan: "STARTER",
  ...
  await prisma.adminSavedListView.create({
    data: {
      adminId: adminUserId,
      name: "Aktif Kurumsal Kiracılar",
      resource: "tenants",
      config: {
        filter: { plan: "ENTERPRISE", status: "ACTIVE" },
        sort: "createdAt:desc",
      },
    },
  });
  ...
  await prisma.adminSensitiveAccessGrant.create({
    data: {
      adminId: adminUserId,
      tenantId,
      fields: ["taxNumber", "email", "phone", "address"],
...
async function seedStarterAndProData(
  ...
  const unitSt = await prisma.unit.create({
    data: { tenantId: st, name: "Adet", code: "AD" },
  });
  const kdv20St = await prisma.taxRate.create({
    data: { tenantId: st, name: "KDV %20", rate: 20 },
  });
  ...
  const whSt = await prisma.warehouse.create({
    data: {
      tenantId: st,
      name: "Mağaza Deposu",
      code: "MAG-01",
  ...
  const locSt = await prisma.location.create({
    data: { tenantId: st, warehouseId: whSt.id, name: "Raf 1", code: "R1" },
  });

  const pSt1 = await prisma.product.create({
    data: {
      tenantId: st,
      code: "ST-001",
      name: "Kablosuz Kulaklık Lite",
      unitId: unitSt.id,
      taxRateId: kdv20St.id,
      purchasePrice: 400,
      salesPrice: 799,
      minStockLevel: 5,
      averageCost: 400,
  ...
  const pSt2 = await prisma.product.create({
    data: {
      tenantId: st,
      code: "ST-002",
  ...
  const custSt = await prisma.contact.create({
    data: {
      tenantId: st,
      type: "CUSTOMER",
      name: "Bireysel Müşteri - Selim Kaya",
      code: "C-ST01",
      city: "İstanbul",
      phone: "+90 555 123 0001",
    },
  });
  ...
  const orderSt = await prisma.salesOrder.create({
    data: {
      ...
      totalTax: 299.6,
      totalGross: 1797.6,
      invoicedAmount: 1797.6,
      items: {
        create: [
          {
            tenantId: st,
            productId: pSt1.id,
            description: "Kablosuz Kulaklık Lite x2",
            quantity: 2,
            unitPrice: 799,
            taxRate: 20,
            taxAmount: 319.6,
  ...
  const unitPr = await prisma.unit.create({
    data: { tenantId: pr, name: "Adet", code: "AD" },
  });
  const kdv20Pr = await prisma.taxRate.create({
    data: { tenantId: pr, name: "KDV %20", rate: 20 },
  });
  await prisma.currency.create({
    data: {
      tenantId: pr,
  ...
  const whPr1 = await prisma.warehouse.create({
    data: {
      tenantId: pr,
      name: "Merkez Depo",
      code: "PR-WH01",
  ...
  const whPr2 = await prisma.warehouse.create({
    data: {
      tenantId: pr,
      name: "Sevkiyat Deposu",
      code: "PR-WH02",
      address: "Hadımköy",
    },
  });
  const locPr1 = await prisma.location.create({
    data: { tenantId: pr, warehouseId: whPr1.id, name: "A-1", code: "A1" },
  });
  const locPr2 = await prisma.location.create({
    data: { tenantId: pr, warehouseId: whPr2.id, name: "B-1", code: "B1" },
  });

  const pPr1 = await prisma.product.create({
    data: {
      tenantId: pr,
      code: "PRO-01",
      name: "Endüstriyel Barkod Okuyucu",
      unitId: unitPr.id,
      taxRateId: kdv20Pr.id,
      purchasePrice: 1500,
      salesPrice: 2400,
      minStockLevel: 10,
      averageCost: 1500,
  ...
  const pPr2 = await prisma.product.create({
    data: {
      tenantId: pr,
      code: "PRO-02",
      name: "Termal Etiket Yazıcı",
      unitId: unitPr.id,
      taxRateId: kdv20Pr.id,
      purchasePrice: 2800,
      salesPrice: 4200,
  ...
  await prisma.stockLevel.create({
    data: {
      tenantId: pr,
      productId: pPr1.id,
      warehouseId: whPr1.id,
      locationId: locPr1.id,
      quantity: 30,
    },
  });
  await prisma.stockLevel.create({
  ...
  const custPr = await prisma.contact.create({
    data: {
      tenantId: pr,
      type: "CUSTOMER",
      name: "Marmara Dağıtım Pazarlama",
      code: "C-PRO01",
      city: "Kocaeli",
      creditLimit: 120000,
      paymentTermDays: 30,
  ...
  await prisma.deliveryNote.create({
    data: {
      ...
      type: "OUTBOUND",
      status: "SHIPPED",
      salesOrderId: orderPr.id,
      contactId: custPr.id,
      warehouseId: whPr1.id,
      date: d("2026-04-29"),
      carrier: "Sürat Kargo",
      trackingNumber: "SK99887766",
      items: {
        create: [
          {
            tenantId: pr,
  ...
  await prisma.marketplaceListing.create({
    data: {
      tenantId: pr,
      integrationId: tyPr.id,
      productId: pPr1.id,
      externalId: "TY-PRO-01",
      price: 2400,
      stock: 25,
      isActive: true,
    },
...
async function runSeed() {
  ...
  // 3. Tenants & Users
  const { tenant, users, planAccounts, expiredTrialAccount } =
    await seedTenant();
  console.log(`  ✓ 3. Ana Kiracı: ${tenant.companyName} (Enterprise)`);
  console.log(`       Kullanıcılar: ${users.map((u) => u.email).join(", ")}`);
  console.log(
    `       Plan Demo Hesapları: ${planAccounts.map(({ user }) => user.email).join(", ")}`,
  );
  console.log(`       Süresi Dolmuş Demo: ${expiredTrialAccount.user.email}`);

  // 4. Master Data
  const master = await seedMasterData(tenant.id);
  console.log(
    "  ✓ 4. Ana Veriler (Birimler, Kategoriler, KDV + Tevkifat, Kurlar, TDHP)",
  );
  ...
  console.log(
    `  ✓ 6. Ürün Kataloğu & Stok Seviyeleri (${products.length} ürün)`,
  );
  ...
  console.log(
    `  ✓ 8. Satış & Finans: ${sales.invoices.length} fatura, ${sales.orders.length} sipariş, ${sales.payments.length} ödeme`,
  );

  // 9. E-Documents (E-Fatura, E-Arşiv, E-İrsaliye)
  await seedEDocuments(tenant.id, sales.invoices, sales.deliveryNotes);
  console.log(
    "  ✓ 9. E-Dönüşüm / E-Belgeler (Onaylı E-Fatura, E-Arşiv, Hatalı Belge & E-İrsaliye)",
  );
  ...
  // 11. Purchasing
  const purchasing = await seedPurchasing(
    tenant.id,
    contacts,
    products,
    master,
  ...
  console.log(
    "  ✓ 11. Satın Alma (Talepler, Siparişler, İrsaliyeli Alış Faturası)",
  );
  ...
  console.log(
    "  ✓ 12. Genel Muhasebe (Mahsup Fişleri, Banka Mutabakatı, Çek/Senet)",
  );

  // 13. HR & Payroll
  await seedHR(tenant.id);
  console.log(
    "  ✓ 13. İnsan Kaynakları & Bordro (Personel, İzin Talepleri, Puantaj, Maaş Bordroları)",
  );

  // 14. Production & Work Centers
  await seedProduction(tenant.id, products, warehouse2);
  console.log(
    "  ✓ 14. Üretim Yönetimi (İş Merkezleri, Kapasite Planı, BOM, İş Emirleri & Maliyet Detayları)",
  );
  ...
  console.log(
    "  ✓ 16. Çoklu Pazaryeri Entegrasyonu (Trendyol, Hepsiburada, Amazon)",
  );
  ...
  console.log(
    "  ✓ 18. Kayıt Yorumları, @Mention, Görevler, E-Postalar & Bildirimler",
  );

  // 19. Automation Engine
  await seedAutomations(tenant.id);
  console.log(
    "  ✓ 19. Otomasyon Motoru (Tetikleyiciler, Kurallar & Çalıştırma Kayıtları)",
  );

  // 20. Attachments (Document Center)
  await seedAttachments(tenant.id, sales, products, service, users[0].id);
  console.log(
    "  ✓ 20. Dosya ve Belge Yönetimi (Fatura PDF, Şartname, Servis Fotoğrafları)",
  );

  // 21. Saved Views & Domain Events
  await seedSavedViewsAndOutbox(tenant.id, users[0].id, "", sales);
  console.log(
    "  ✓ 21. Kayıtlı Tablo Görünümleri & Domain Event Outbox Kuyruğu",
  );

  // 22. SaaS Subscriptions & Billing
  await seedBilling(
    tenant.id,
    planAccounts[0].tenant.id,
    planAccounts[1].tenant.id,
    platformAdmin.id,
  );
  console.log(
    "  ✓ 22. SaaS Abonelikleri, Faturalar, Kuponlar & Özel Fiyat Talepleri",
  );
  ...
  console.log(
    "  ✓ 24. Kurumsal Yönetişim (API Anahtarları, Davetiyeler, Feature Override, Audit Log, AI Logları)",
  );

  // 25. Platform Observability & Security
  await seedPlatformHealth(tenant.id, platformAdmin.id);
  console.log(
    "  ✓ 25. Gözlemlenebilirlik (SLO, Metrikler, Alarmlar), Güvenlik, Olay Kaydı & Felaket Kurtarma",
  );

  // 26. Starter & Professional Demo Data
  await seedStarterAndProData(planAccounts[0].tenant, planAccounts[1].tenant);
  console.log(
    "  ✓ 26. Starter ve Professional Demo Kiracıları İçin Özel Veriler",
  );
  ...
  console.log(
    "     admin@axondemo.com    / demo1234  (Yönetici - Tüm Modüller)",
  );
  ...
  console.log(
    "     starter@axondemo.com  / demo1234  (Starter Plan - Sade Perakende)",
  );
  console.log(
    "     pro@axondemo.com      / demo1234  (Professional Plan - Toptan Ticaret)\n",
  );
  console.log(
    "     expired@axondemo.com  / demo1234  (Süresi Dolmuş Professional Trial - Ödeme Akışı)\n",
  );

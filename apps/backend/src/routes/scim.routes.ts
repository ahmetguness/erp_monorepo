...
function userEmail(payload: ScimUserPayload): string | undefined {
  return (
    payload.userName ??
    payload.emails?.find((email) => email.primary)?.value ??
...
function scimUserResource(
  ...
  return {
    ...
    groups: tenantUser?.roleRef
      ? [{ value: tenantUser.roleRef.id, display: tenantUser.roleRef.name }]
      : [],
  };
}
...
async function resolveScimRoleId(
  tenantId: string,
  payload: ScimUserPayload,
): Promise<string | null> {
  const roleSync = await prisma.tenantSetting.findUnique({
    where: {
      tenantId_key: { tenantId, key: "security.scim.role_sync.enabled" },
    },
    select: { value: true },
  });
  ...
  const matchedMapping = mappings.find((mapping) =>
    externalGroups.some(
      (group) =>
        ...
        mapping.group.toLocaleLowerCase("tr-TR"),
    ),
  );
  if (matchedMapping)
    return ensureRoleBelongsToTenant(tenantId, matchedMapping.roleId);
...
function roleSyncPatchPayload(payload: ScimUserPayload): ScimUserPayload {
  ...
  for (const operation of payload.Operations ?? []) {
    ...
    if (isRecord(operation.value)) {
      if (Array.isArray(operation.value.groups))
        patched.groups = readMembers(operation.value.groups);
      if (Array.isArray(operation.value.roles))
        patched.roles = readMembers(operation.value.roles);
      if (typeof operation.value.active === "boolean")
        patched.active = operation.value.active;
    }
    if (
      (operation.path === "groups" || operation.path === "roles") &&
    ...
    if (operation.path === "active" && typeof operation.value === "boolean")
      patched.active = operation.value;
  }
  return patched;
}
...
scimRoutes.use("*", async (c, next) => {
  ...
  if (!authHeader?.startsWith("Bearer "))
    return scimError(c, "SCIM Bearer token eksik.", 401);
  ...
  if (tenant?.plan !== Plan.ENTERPRISE)
    return scimError(c, "SCIM sadece Enterprise plan icindir.", 403);

  const enabled = await prisma.tenantSetting.findUnique({
    where: {
      tenantId_key: {
        tenantId: setting.tenantId,
        key: "security.scim.enabled",
      },
    },
    select: { value: true },
  });
  if (enabled?.value !== "true")
    return scimError(c, "SCIM provizyonlama etkin degil.", 403);
...
scimRoutes.get("/Users", async (c) => {
  ...
  return c.json({
    ...
    Resources: tenantUsers.map((tenantUser) =>
      scimUserResource(tenantUser.user, tenantUser),
    ),
  });
});
...
scimRoutes.put("/Users/:id", async (c) => {
  ...
  const resource = await prisma.$transaction(async (tx) => {
    ...
    await tx.tenantUser.update({
      ...
      data: { isActive: active, ...(roleId !== null && { roleId }) },
    });
    return tx.tenantUser.findUnique({
...
scimRoutes.patch("/Groups/:id", async (c) => {
  ...
  for (const operation of payload.Operations ?? []) {
    ...
    const members =
      isRecord(operation.value) && Array.isArray(operation.value.members)
    ...
    if (op === "remove") {
      await prisma.tenantUser.updateMany({
        where: { tenantId, userId: { in: memberIds }, roleId: id },
        data: { roleId: null },
      });
    }
  }

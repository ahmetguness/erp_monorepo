...
import { applyCoupon, createCoupon, deactivateCoupon, decideCustomPrice, getRevenueOverview, getSubscriptionSnapshot, ingestProviderEvent, listCoupons, listCustomPrices, quotePlanChange, reconcileEntitlements, requestCustomPrice, setCouponActive } from '../../../subscription-operations/subscription-operations.service.js';
...
export const AdminSubscriptionOperationsController = {
  ...
  async updateCouponStatus(c: Context) {
    const input = parse(z.object({ isActive: z.boolean() }).strict().safeParse(await body(c)));
    await setCouponActive(requireParam(c, 'id'), input.isActive);

...
import {
  StarterCsvImportService,
  type StarterCsvImportEntity,
  type StarterCsvImportInput,
} from "../../../../services/starter-csv-import.service.js";
...
function parseEntityFromPath(path: string): StarterCsvImportEntity | null {
  ...
  if (path.includes("/starter-import/opening-stock/")) return "opening-stock";
  if (path.includes("/starter-import/prices/")) return "prices";
  return null;
}
...
function requireEntity(c: Context): StarterCsvImportEntity | Response {
  const entity =
    parseEntity(c.req.param("entity")) ?? parseEntityFromPath(c.req.path);
  if (!entity)
    return c.json(
      new ValidationError("Gecersiz Starter import varligi.").toJSON(),
      400,
    );
  return entity;
}
...
export const StarterCsvImportController = {
  async template(c: Context): Promise<Response> {
    ...
    return csvResponse(
      service.buildTemplateCsv(entity),
      `starter-${entity}-import-template.csv`,
    );
  },
  ...
  async preview(c: Context): Promise<Response> {
    ...
    const preview = await new StarterCsvImportService(prisma).preview(
      entity,
      tenantId,
      body,
    );
  ...
  async commit(c: Context): Promise<Response> {
    ...
    const result = await new StarterCsvImportService(prisma).commit(
      entity,
      tenantId,
      userId,
      body,

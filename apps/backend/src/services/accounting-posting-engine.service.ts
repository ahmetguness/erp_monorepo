...
function normalizeSource(value: string | undefined): PostingSource {
  if (value === "INVOICE" || value === "PAYMENT" || value === "STOCK")
    return value;
  return "ALL";
}
...
export function parsePostingEngineOptions(
  input: Record<string, unknown>,
  userId?: string | null,
): PostingEngineRunOptions {
  const source =
    typeof input.source === "string" ? normalizeSource(input.source) : "ALL";
  const limit = normalizeLimit(
    typeof input.limit === "number" ? input.limit : undefined,
  );
  const postImmediately =
    typeof input.postImmediately === "boolean" ? input.postImmediately : true;
...
export class AccountingPostingEngineService {
  ...
  async run(
  ...
  ): Promise<PostingEngineRunResult> {
    ...
    if (options.source === "ALL" || options.source === "INVOICE") {
      const invoices = await this.findInvoiceCandidates(
        tenantId,
        options.limit,
      );
      for (const invoice of invoices) {
        items.push(
          await this.postInvoice(tenantId, invoice, mappings, options),
        );
      }
    }

    if (options.source === "ALL" || options.source === "PAYMENT") {
      const payments = await this.findPaymentCandidates(
        tenantId,
        options.limit,
      );
      for (const payment of payments) {
        items.push(
          await this.postPayment(tenantId, payment, mappings, options),
        );
      }
    }

    if (options.source === "ALL" || options.source === "STOCK") {
      const stockCount = await this.db.stockMovement.count({
        where: { tenantId },
      });
      if (stockCount > 0) {
        items.push({
          ...
          journalEntryId: null,
          message:
            "Stok posting için JournalEntryType enumunda AUTO_STOCK bulunmadığından güvenli şekilde atlandı.",
        });
      }
    }
  ...
  async postInvoiceById(
    tenantId: string,
    invoiceId: string,
    userId?: string | null,
    idempotencyKey?: string,
  ): Promise<PostingEngineItemResult> {
    const existing = await this.db.journalEntry.findFirst({
    ...
    if (existing) {
      if (idempotencyKey && existing.idempotencyKey && existing.idempotencyKey !== idempotencyKey) {
        throw new Error("Muhasebe fisi daha once farkli bir idempotencyKey ile olusturulmus.");
      }
      ...
      return {
        ...
        message: "Fatura muhasebe fisi daha once olusturulmus.",
      };
    }
    return this.postInvoice(
      tenantId,
      invoice,
      await this.resolveMappings(tenantId),
      {
        source: "INVOICE",
        limit: 1,
  ...
  private async resolveMappings(
    tenantId: string,
  ): Promise<Map<PostingRole, { id: string; name: string; code: string }>> {
    ...
    >();
    for (const [role, code] of Object.entries(ROLE_DEFAULT_CODES) as Array<
      [PostingRole, string]
    >) {
      const account =
        accounts.find((item) => item.code === code) ??
        accounts.find((item) => item.code.startsWith(code));
      if (account) mappings.set(role, account);
    }
    return mappings;
  }

  private toMappingStatus(
    mappings: Map<PostingRole, { id: string; name: string; code: string }>,
  ): PostingEngineMappingStatus[] {
    return (
      Object.entries(ROLE_DEFAULT_CODES) as Array<[PostingRole, string]>
    ).map(([role, code]) => {
  ...
  private invoiceLines(
    ...
    if (purchase) {
      ...
      const lines: PostingLineDraft[] = [
        {
          role: "PURCHASE_INVENTORY",
          debit: inventoryDebit,
          credit: inventoryCredit,
          description: "Satın alma stok/masraf",
        },
        {
          role: "VAT_RECEIVABLE",
          debit: vatDebit,
  ...
  private async postPayment(
    ...
    const contactRole: PostingRole =
      payment.direction === "SEND" ? "SUPPLIER_PAYABLE" : "CUSTOMER_RECEIVABLE";
    const lines: PostingLineDraft[] =
  ...
  private async createJournalEntry(
    ...
    const missingRole = params.lines.find(
      (line) => !params.mappings.has(line.role),
    )?.role;
    if (missingRole) {
      if (params.options.emitDomainEvent !== false) await this.publishPostingFailed(
        tenantId,
        params.refType,
        params.refId,
        `${missingRole} için hesap planı mapping'i bulunamadı.`,
    ...
    const debit = roundMoney(
      params.lines.reduce((sum, line) => sum + line.debit, 0),
    );
    ...
    if (Math.abs(debit - credit) > 0.01) {
      if (params.options.emitDomainEvent !== false) await this.publishPostingFailed(
        ...
        params.refType,
        params.refId,
        `Fiş dengeli değil. Borç ${debit}, alacak ${credit}.`,
        params.options.userId,
      );
      return {
        source: params.source,
    ...
    try {
      ...
      const entry = await this.db.journalEntry.create({
        data: {
          ...
          postedById: params.options.postImmediately
            ? (params.options.userId ?? null)
            : null,
          createdById: params.options.userId ?? null,
          idempotencyKey: params.options.idempotencyKey ?? null,
          lines: {
            create: params.lines.map((line, index) => {
              const account = params.mappings.get(line.role);
              if (!account)
                throw new Error(`${line.role} mapping'i bulunamadı.`);
              return {
                tenantId,
      ...
      if (params.options.emitDomainEvent !== false) {
      ...
      return {
        ...
        status: "POSTED",
        journalEntryId: entry.id,
        message: params.options.postImmediately
          ? "Otomatik fiş oluşturuldu ve onaylandı."
          : "Otomatik fiş taslak oluşturuldu.",
      };
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Bilinmeyen posting hatası.";
      if (params.options.emitDomainEvent !== false) await this.publishPostingFailed(
        tenantId,
        params.refType,
        params.refId,

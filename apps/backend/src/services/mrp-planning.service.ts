...
interface WorkCenterLookup extends ProductLookup {
  capacity: unknown;
}
...
function productRef(product: ProductLookup): MrpProductRef {
...
export async function getMrpPlanning(
  ...
  const products = await getProductsByIds(db, tenantId, salesDemandByProduct.keys());
  ...
  for (const [productId, demandQty] of salesDemandByProduct.entries()) {
    const product = products.get(productId);
    if (!product) continue;
    const stockQty = stockByProduct.get(productId) ?? 0;
    const openWorkOrderQty = openWorkOrderSupplyByProduct.get(productId) ?? 0;
    const recommendedQty = Math.max(0, demandQty - stockQty - openWorkOrderQty);
    ...
    if (!bom) {
      ...
      const purchaseQty = Math.max(0, recommendedQty - openPurchaseQty);
  ...
  for (const row of materialRequirements.values()) {
    ...
    const recommendedQty = Math.max(0, row.quantity - stockQty - openPurchaseQty);
    if (recommendedQty <= 0) continue;
    purchaseRecommendations.push({
      product: productRef(product),
      source: 'bom_component',
      ...(parentProduct ? { parentProduct: productRef(parentProduct) } : {}),
      grossRequirementQty: roundQty(row.quantity),
      stockQty: roundQty(stockQty),
      openPurchaseQty: roundQty(openPurchaseQty),
      recommendedQty: roundQty(recommendedQty),
    });
  ...
  return {
    summary: {
      horizonDays,
      demandProducts: salesDemandByProduct.size,

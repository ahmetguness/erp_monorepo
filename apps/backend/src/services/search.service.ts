import { EntityType, MailDeliveryStatus, PermissionAction, Prisma, PrismaClient } from '@prisma/client';
...
export type SearchResultKind = 'record' | 'action';
...
function buildActions(can: (action: PermissionAction, module: string) => boolean): SearchResult[] {
  ...
  if (can(PermissionAction.CREATE, 'sales') || can(PermissionAction.CREATE, 'invoicing')) {
    actions.push(actionResult({ id: 'new-sales-quote', module: 'sales', title: 'Yeni teklif olustur', subtitle: 'Satis teklifi formunu ac', href: '/dashboard/sales-orders/quotes/new' }));
  }
  if (can(PermissionAction.CREATE, 'contacts')) {
    actions.push(actionResult({ id: 'new-contact', module: 'contacts', title: 'Yeni musteri / cari', subtitle: 'Cari karti olustur', href: '/dashboard/contacts/new' }));
  }
  if (can(PermissionAction.CREATE, 'accounting')) {
    actions.push(actionResult({ id: 'new-payment', module: 'accounting', title: 'Odeme ekle', subtitle: 'Tahsilat veya odeme kaydi olustur', href: '/dashboard/payments/new' }));
  }
  if (can(PermissionAction.CREATE, 'mail')) {
    actions.push(actionResult({ id: 'send-mail', module: 'mail', title: 'Mail gonder', subtitle: 'Mail Merkezini ac', href: '/dashboard/mail' }));
  }
  actions.push(actionResult({ id: 'create-task', module: 'workflow', title: 'Gorev olustur', subtitle: 'Is Akisi Merkezine git', href: '/dashboard/workflow' }));
  return actions;
}
...
export class SearchService {
  ...
  async global(input: GlobalSearchInput): Promise<GlobalSearchResult | ForbiddenError> {
    const { tenantId, userId, query, limit } = input;
    const permissions = await getTenantPermissionContext(tenantId, userId);
    if (!permissions) return new ForbiddenError("Bu tenant'a erisiminiz yok.");
    ...
    if (query.length < 2) {
      return { data: results.slice(0, limit), meta: { query, total: results.length } };
    }
    ...
    if (can(PermissionAction.READ, 'mail')) {
      const email = await getTenantUserEmail(this.db, tenantId, userId);
      const mails = email
        ? await this.db.mailMessage.findMany({
            where: {
              tenantId,
      ...
      results.push(...mails.map((mail): SearchResult => result({
        ...
        href: '/dashboard/mail',
        status: mail.status === MailDeliveryStatus.FAILED ? 'Basarisiz' : mail.status,
    ...
    if (can(PermissionAction.READ, 'attachments')) {
      ...
      results.push(...attachments.map((attachment): SearchResult => result({
        ...
        subtitle: attachment.category ?? 'Dokuman',
    ...
    results.push(...tasks.map((task): SearchResult => result({
      ...
      meta: [{ label: 'Oncelik', value: task.priority }, { label: 'Tip', value: task.type }],
    })));
